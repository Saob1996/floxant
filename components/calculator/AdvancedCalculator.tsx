...
export default function AdvancedCalculator({ dic }: { dic?: any }) {
  ...
  useEffect(() => {
    const updateEstimate = () => {
      ...
      if (store.serviceType === 'umzug') {
        estimate = calculateUmzugAdvanced(store.umzugData, store.baseDetails, dic);
      } else if (store.serviceType === 'reinigung') {
        estimate = calculateReinigungAdvanced(store.reinigungData, dic);
      } else if (store.serviceType === 'entsorgung') {
        estimate = calculateEntsorgungAdvanced(store.entsorgungData, dic);
      }
      ...
      <div className="flex-[1.2] w-full transition-all duration-700 ease-in-out">
        <div className="bg-secondary/40 rounded-3xl p-6 lg:p-10 border border-white/5 shadow-lg relative overflow-hidden backdrop-blur-sm">
          <div className="relative z-10">
            ...
            {store.serviceType === 'umzug' && <UmzugForm dic={dic} />}
            {store.serviceType === 'reinigung' && <ReinigungForm dic={dic} />}
            {store.serviceType === 'entsorgung' && <EntsorgungForm dic={dic} />}
          </div>
        </div>
      </div>
