...
export default function EntsorgungForm({ dic }: { dic?: any }) {
  const SECTIONS = [
    { id: 'access', label: dic?.calculator.access_disassembly, icon: Route },
    { id: 'details', label: dic?.calculator.safety_urgency, icon: AlertTriangle },
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-6 space-y-4">
        <h3 className="text-xs font-bold text-foreground/70 uppercase tracking-[0.15em] flex items-center gap-2 mb-4">
          <Trash2 size={14} className="text-primary" />{dic?.calculator.waste_volume_title}
        </h3>
        
        <div className="space-y-2">
          <label className="text-[11px] text-muted-foreground tracking-wide uppercase">{dic?.calculator.estimated_waste_volume}</label>
          <input 
            type="number" 
        ...
        <label className="flex items-start gap-4 p-4 rounded-xl border border-white/5 bg-white/[0.02] cursor-pointer hover:bg-white/[0.04] transition-colors">
          ...
          <div>
              <span className="text-sm font-medium text-foreground block">{dic?.calculator.uncertain_waste_volume}</span>
              <span className="text-[11px] text-muted-foreground block mt-1 leading-relaxed">{dic?.calculator.uncertain_waste_volume_desc}</span>
          </div>
        </label>

        <div className="space-y-2 pt-2">
          <label className="text-[11px] text-muted-foreground tracking-wide uppercase">{dic?.calculator.waste_types}</label>
          <div className="flex flex-wrap gap-2">
            {[
              { id: 'sperrmuell', label: dic?.calculator.bulky_waste },
              { id: 'elektroschrott', label: dic?.calculator.e_waste },
              { id: 'bauschutt', label: dic?.calculator.construction_waste },
              { id: 'gruenschnitt', label: dic?.calculator.green_waste },
              { id: 'hausmuell', label: dic?.calculator.household_waste },
              { id: 'altmetall', label: dic?.calculator.scrap_metal },
              { id: 'mischabfall', label: dic?.calculator.mixed_waste }
            ].map(cat => {
                ...
                >
                  ...
                  {section.id === 'access' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                      <div className="space-y-2">
                        <label className="text-[11px] text-muted-foreground uppercase tracking-wider">{dic?.calculator.access_difficulty}</label>
                        <select 
                          value={entsorgungData.accessDifficulty} 
                          onChange={e => updateEntsorgungData({ accessDifficulty: e.target.value as any })}
                          className="w-full bg-background border border-white/10 rounded-md p-3 text-sm outline-none focus:border-white/30 transition-colors"
                        >
                          <option value="einfach">{dic?.calculator.easy}</option>
                          <option value="mittel">{dic?.calculator.medium}</option>
                          <option value="schwer">{dic?.calculator.difficult}</option>
                        </select>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[11px] text-muted-foreground uppercase tracking-wider">{dic?.calculator.loading_path_meters}</label>
                        <input 
                          type="number" 
                      ...
                      <label className="flex items-center gap-3 p-3 rounded-lg border border-border bg-background/50 cursor-pointer md:col-span-2">
                        ...
                        <div>
                          <span className="text-sm font-medium block">{dic?.calculator.disassembly_required}</span>
                          <span className="text-xs text-muted-foreground">{dic?.calculator.disassembly_required_desc}</span>
                        </div>
                      </label>
                    </div>
                  )}
                  ...
                  {section.id === 'details' && (
                    <div className="grid grid-cols-1 gap-4">
                      <label className="flex items-start gap-4 p-4 rounded-xl bg-amber-500/5 border border-amber-500/20 cursor-pointer">
                        ...
                        <div>
                          <span className="text-sm font-medium text-amber-500/90 block">{dic?.calculator.hazard_materials}</span>
                          <span className="text-xs text-muted-foreground">{dic?.calculator.hazard_materials_desc}</span>
                        </div>
                      </label>
                      
                      <div className="space-y-2">
                        <label className="text-xs text-muted-foreground uppercase tracking-wider">{dic?.calculator.urgency}</label>
                        <select 
                        ...
                        >
                          <option value="flexibel">{dic?.calculator.standard_flexible}</option>
                          <option value="dringend">{dic?.calculator.urgent_express}</option>
                        </select>
                      </div>
                    </div>
                  )}
                  ...
                  {section.id === 'notes' && (
                    <div className="space-y-2">
                      ...
                      <textarea 
                        ...
                        placeholder={dic?.calculator.waste_notes_placeholder}
