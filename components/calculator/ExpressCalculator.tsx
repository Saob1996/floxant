...
export default function ExpressCalculator({ dic }: { dic?: any }) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 md:p-8 rounded-3xl relative overflow-hidden bg-[#0B0B12] border border-white/5 shadow-2xl">
      ...
      <div className="absolute top-0 start-/4 w-96 h-96 bg-blue-600/20 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 end-/4 w-96 h-96 bg-violet-600/20 rounded-full blur-[120px] pointer-events-none" />
        ...
        <AnimatePresence mode="wait">
          ...
          {activeStep === 0 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <ServiceCard icon={Truck} title={dic?.nav?.service_umzug} onClick={() => handleServiceSelect('umzug')} active={store.serviceType === 'umzug'} />
                <ServiceCard icon={Sparkles} title={dic?.nav?.service_reinigung} onClick={() => handleServiceSelect('reinigung')} active={store.serviceType === 'reinigung'} />
                <ServiceCard icon={Trash2} title={dic?.nav?.service_entruempelung} onClick={() => handleServiceSelect('entsorgung')} active={store.serviceType === 'entsorgung'} />
              </div>
              <button 
              ...
              >
                {dic?.common?.next} <ArrowRight size={18} />
              </button>
            </m.div>
          )}
          ...
          {activeStep === 2 && (
            ...
            >
              ...
              <h2 className="text-white/60 text-lg mb-2">{dic?.calculator?.prognosis_label}</h2>
              ...
              >
                ...
                <span className="relative flex items-center justify-center gap-2">
                  {dic?.calculator?.start_now} <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                </span>
              </button>
