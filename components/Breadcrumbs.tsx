     ...
     {fullItems.map((item, index) => (
      <li key={`${item.label}-${index}`} className="flex items-center gap-2">
       ...
       {item.href && index < fullItems.length - 1 ? (
        <Link href={item.href} className="text-slate-600 transition-all hover:text-primary hover:tracking-widest">
         {item.label}
        </Link>
       ) : (
        <span className="line-clamp-1 text-slate-600">{item.label}</span>
       )}
      </li>
     ))}
    </ol>
