...
import { Building2, Calendar, CheckCircle2, Loader2, Mail, MapPin, PackageOpen, Phone, Send, User } from "lucide-react";
...
export function BusinessDisposalForm() {
  ...
  return (
    <div className="rounded-[2.2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.065),rgba(255,255,255,0.025))] p-6 shadow-2xl">
      <AnimatePresence mode="wait">
        {isSuccess ? (
          ...
          >
            ...
            <h3 className="text-2xl font-semibold text-white">Firmenanfrage gesendet</h3>
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-white/50">
              FLOXANT prüft jetzt Volumen, Materialarten, Zugang und Terminfenster. Wir melden uns mit einer sauberen B2B-Einordnung.
            </p>
          </m.div>
        ) : (
          <m.form
          ...
          >
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-300">B2B-Anfrage</div>
              <h2 className="mt-2 text-3xl font-semibold tracking-tight text-white">Firmenentsorgung prüfen lassen</h2>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Input icon={PackageOpen} label="Menge / Volumen" value={form.volumeEstimate} onChange={(value) => update("volumeEstimate", value)} placeholder="z. B. 20 Arbeitsplätze, 30 m³" />
              <Input icon={PackageOpen} label="Materialarten" value={form.materialTypes} onChange={(value) => update("materialTypes", value)} placeholder="Möbel, Kartons, Verpackung, Regale..." />
            </div>
            <Input icon={MapPin} label="Zugang / Ladeweg" value={form.access} onChange={(value) => update("access", value)} placeholder="Etage, Aufzug, Rampe, Hof, Zeitfenster..." />
            <Input icon={PackageOpen} label="Preisvorstellung optional" value={form.budget} onChange={(value) => update("budget", value)} placeholder="z. B. 850 EUR" />

            <label className="block">
              <span className="mb-2 block text-[10px] font-bold uppercase tracking-[0.16em] text-white/30">Hinweis optional</span>
              <textarea
                value={form.note}
                onChange={(event) => update("note", event.target.value)}
                placeholder="Was soll weg? Gibt es interne Zeitfenster, Gebäuderegeln oder sensible Bereiche?"
                className="h-24 w-full resize-none rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm font-medium text-white outline-none transition focus:border-cyan-300/40 focus:bg-black/35"
              />
            </label>
            ...
            <button
              ...
              className="group flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-cyan-300 text-sm font-bold uppercase tracking-[0.14em] text-slate-950 transition hover:bg-cyan-200 disabled:opacity-60"
            >
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-12 w-full rounded-2xl border border-white/10 bg-black/25 px-4 text-sm font-medium text-white outline-none transition focus:border-cyan-300/40 focus:bg-black/35"
      />
    </label>
  );
}
