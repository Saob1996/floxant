        ...
        <div className="mt-8 divide-y divide-slate-200 rounded-lg border border-slate-200 bg-slate-50">
          {items.map((item) => (
            <div key={item.q} className="p-5">
              <h3 className="text-base font-black text-slate-950">{germanText(item.q, item.q)}</h3>
              <p className="mt-3 text-sm font-semibold leading-7 text-slate-600">{germanText(item.a, item.a)}</p>
            </div>
          ))}
        </div>
      </div>
