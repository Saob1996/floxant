    ...
    <section className="px-4 py-12 sm:px-6" data-component="EffortFactorsPanel" data-effort-group={group}>
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950">{title || definition.title}</h2>
          <p className="mt-3 text-base leading-8 text-slate-700">{intro || definition.intro}</p>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {factors.map((factor) => (
            <article key={factor.key} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/5">
              ...
              <h3 className="mt-4 text-lg font-black text-slate-950">{factor.label}</h3>
              <p className="mt-2 text-sm leading-7 text-slate-700">{factor.whyItMatters}</p>
              <p className="mt-3 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs font-bold leading-5 text-blue-950">
                Hilfreich: {factor.helpfulInput}
              </p>
            </article>
          ))}
        </div>
