    ...
    <section className="border-b border-slate-200 bg-white px-5 py-14 sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950 sm:text-5xl">{germanizeText(title)}</h2>
          <p className="mt-5 text-base font-semibold leading-8 text-slate-600">{germanizeText(intro)}</p>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {services.map((service) => (
            <article key={service.title} className="rounded-lg border border-slate-200 bg-slate-50 p-5">
              <h3 className="text-lg font-black tracking-normal text-slate-950">{germanizeText(service.title)}</h3>
              <p className="mt-3 text-sm font-semibold leading-7 text-slate-600">{germanizeText(service.text)}</p>
