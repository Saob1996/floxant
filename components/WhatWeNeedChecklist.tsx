      ...
      <div className="mx-auto grid max-w-7xl gap-7 lg:grid-cols-[0.76fr_1.24fr]">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950">
            {title || "Mit diesen Angaben wird die Anfrage besser buchbar"}
          </h2>
          <p className="mt-4 text-base leading-8 text-slate-700">
            {intro || "Fotos, grobe Eckdaten und ein ehrlicher Pruefgrund reichen oft fuer den ersten Schritt."}
          </p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          {items.map((item) => (
            <div key={item.key} className="flex gap-3 rounded-lg border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-700 shadow-sm shadow-slate-950/5">
              ...
              <span>
                <strong className="text-slate-950">{item.label}:</strong> {item.helpfulInput}
              </span>
            </div>
          ))}
        </div>
