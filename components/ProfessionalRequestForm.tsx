...
function locationLabel(context: RequestContext) {
  ...
  if (context.location === "unsicher") return context.locationHint || "Anderer Ort";
  return "";
}
...
export function ProfessionalRequestForm({
  ...
  const [startLocation, setStartLocation] = useState(() =>
    context.location === "regensburg" ? "Regensburg" : context.locationHint,
  );
  ...
  useEffect(() => {
    ...
    if (
      previous.location === context.location &&
      previous.locationHint === context.locationHint &&
      previous.serviceKey === context.serviceKey
    ) {
      return;
    }

    const locationChanged =
      previous.location !== context.location || previous.locationHint !== context.locationHint;
    ...
    if (locationChanged) {
      ...
      setStartLocation(context.location === "regensburg" ? "Regensburg" : context.locationHint);
    }
  ...
  function detailErrors() {
    ...
    if (group === "moving") {
      ...
      if (group === "clearance" && !areaSize.trim()) {
        next.areaSize = "Bitte ungefähre Größe oder Umfang angeben.";
      } else if (group === "cleaning") {
        const numericArea = Number(areaSize.trim().replace(",", "."));
        if (!areaSize.trim() || !Number.isFinite(numericArea) || numericArea < 1) {
          next.areaSize = "Bitte eine Fläche ab 1 m² angeben.";
        }
      }
    }
    return next;
  ...
  const locationCoreFields = ["offer_check", "general"].includes(group) ? (
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <Field id="request-area" label={group === "clearance" ? "Ungefähre Größe oder Umfang" : "Ungefähr zu bearbeitende Fläche (m²)"} required error={errors.areaSize}>
        <input id="request-area" name="areaSize" type={group === "cleaning" ? "number" : "text"} min={group === "cleaning" ? 1 : undefined} step={group === "cleaning" ? "any" : undefined} inputMode={group === "cleaning" ? "decimal" : undefined} value={areaSize} onChange={(event) => { setAreaSize(event.target.value); clearFieldErrors("areaSize"); }} className={inputClass} aria-required="true" aria-invalid={Boolean(errors.areaSize)} aria-describedby={errors.areaSize ? "request-area-error" : undefined} placeholder={group === "cleaning" ? "z. B. 1, 75 oder 2500" : "z. B. 85 m² oder 3 Räume"} />
      </Field>
      {group === "clearance" ? (
        <>
        ...
        <>
          <Field id="request-scope" label="Räume, Bereiche oder gewünschter Reinigungsumfang (optional)" error={errors.scope}>
            <input id="request-scope" name="scope" value={scope} onChange={(event) => { setScope(event.target.value); clearFieldErrors("scope"); }} className={inputClass} aria-invalid={Boolean(errors.scope)} aria-describedby={errors.scope ? "request-scope-error" : undefined} />
          </Field>
