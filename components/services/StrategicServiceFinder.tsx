...
export function StrategicServiceFinder({
  ...
  const steps: FinderStep[] = [
    ...
    ...(answers.category === "moving"
      ? [{ key: "start" as const, title: copy.route, routeFields: true }]
      : []),
    ...
    { key: "contactMethod", title: copy.contactMethod, options: [{ value: "phone", label: copy.phone }, { value: "whatsapp", label: copy.whatsapp }, { value: "email", label: copy.email }] },
  ];
  ...
  const hasAnswer = current.routeFields
    ? Boolean(answers.start.trim() && answers.destination.trim())
  ...
  function withFinderParams(href: string) {
    const [hrefWithoutHash, hash] = href.split("#", 2);
    const separator = hrefWithoutHash.includes("?") ? "&" : "?";
    const params = new URLSearchParams({ preferredContact: answers.contactMethod });
    if (answers.category === "moving") {
      params.set("start", answers.start);
      params.set("destination", answers.destination);
    }
    return `${hrefWithoutHash}${separator}${params.toString()}${hash ? `#${hash}` : ""}`;
  }
  ...
  if (completed && result.primary) {
    return (
      <section className="rounded-3xl border border-cyan-300 bg-white p-5 shadow-sm sm:p-8">
        ...
        {answers.category === "moving" ? <p className="mt-5 rounded-2xl bg-slate-50 p-4 font-medium text-slate-700"><span className="font-black text-slate-950">{copy.routeSummary}:</span> {answers.start} → {answers.destination}</p> : null}
        ...
        {(result.primary.faqLinks.length || result.primary.articleLinks.length) ? (
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {result.primary.faqLinks.slice(0, 2).map((link) => <Link key={link.href} href={link.href} prefetch={false} className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-black text-blue-900 outline-none focus-visible:ring-2 focus-visible:ring-cyan-600">{link.label}</Link>)}
            {result.primary.articleLinks.slice(0, 2).map((link) => <Link key={link.href} href={link.href} prefetch={false} className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-black text-blue-900 outline-none focus-visible:ring-2 focus-visible:ring-cyan-600">{link.label}</Link>)}
          </div>
        ) : null}
        <div className="mt-7 grid gap-3 sm:grid-cols-2"><Link href={withFinderParams(result.primary.ctaHref)} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-cyan-400 px-5 text-sm font-black text-slate-950 outline-none hover:bg-cyan-300 focus-visible:ring-2 focus-visible:ring-cyan-700">{copy.cta}<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><Link href={result.primary.canonicalRoute} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-slate-300 px-5 text-sm font-black text-slate-950 outline-none hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-cyan-600">{copy.details}</Link></div>
        ...
        {current.routeFields ? (
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <label className="grid gap-2 text-sm font-black text-slate-800">
              {copy.start}
              <input value={answers.start} onChange={(event) => setAnswers((currentAnswers) => ({ ...currentAnswers, start: event.target.value }))} autoComplete="address-level2" placeholder={copy.startPlaceholder} className="min-h-12 rounded-xl border border-slate-300 bg-white px-4 font-medium outline-none focus-visible:border-cyan-700 focus-visible:ring-2 focus-visible:ring-cyan-600" />
            </label>
            <label className="grid gap-2 text-sm font-black text-slate-800">
              {copy.destination}
              <input value={answers.destination} onChange={(event) => setAnswers((currentAnswers) => ({ ...currentAnswers, destination: event.target.value }))} autoComplete="off" placeholder={copy.destinationPlaceholder} className="min-h-12 rounded-xl border border-slate-300 bg-white px-4 font-medium outline-none focus-visible:border-cyan-700 focus-visible:ring-2 focus-visible:ring-cyan-600" />
            </label>
          </div>
        ) : (
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {(current.options ?? []).map((option) => {
            ...
            return <button key={option.value} type="button" aria-pressed={selected} onClick={() => select(option.value, Boolean(current.multi))} className={`min-h-14 rounded-2xl border px-4 py-3 text-left font-bold outline-none focus-visible:ring-2 focus-visible:ring-cyan-600 ${selected ? "border-cyan-700 bg-cyan-50 text-cyan-950" : "border-slate-300 bg-white text-slate-800 hover:border-cyan-500 hover:bg-slate-50"}`}>{option.label}</button>;
          })}
        </div>
        )}
      </fieldset>
...
const deCopy = {
  region: "In welcher Region liegt der Auftrag?", audience: "Ist die Anfrage privat oder gewerblich?", category: "Worum geht es hauptsächlich?", route: "Wo beginnt und endet der Umzug?", start: "Startort oder Start-PLZ", destination: "Zielort oder Ziel-PLZ", startPlaceholder: "z. B. Regensburg", destinationPlaceholder: "z. B. Nürnberg", routeSummary: "Gewählte Strecke", cadence: "Einmalig oder regelmäßig?", objectType: "Welche Objektart passt am besten?", situation: "Welche Situation beschreibt den Auftrag?", requirements: "Was ist besonders wichtig?", hasOffer: "Liegt bereits ein Angebot vor?", hasPhotos: "Sind Fotos vorhanden?", nextStep: "Was ist der gewünschte nächste Schritt?", contactMethod: "Wie möchten Sie kontaktiert werden?", phone: "Telefon", whatsapp: "WhatsApp", email: "E-Mail", private: "Privat", business: "Gewerblich", cleaning: "Reinigung", moving: "Umzug oder Transport", clearance: "Räumung oder Auflösung", offer: "Angebot prüfen", oneOff: "Einmalig", recurring: "Regelmäßig", standard: "Normal planbarer Auftrag", deadline: "Frist oder kurzfristige Änderung", sensitive: "Sensible oder diskrete Situation", combined: "Mehrere Leistungen kombinieren", access: "Zugang, Schlüssel oder Alarm", handover: "Übergabe oder Auszug", coordination: "Mehrere Beteiligte koordinieren", none: "Keine besondere Anforderung", yes: "Ja", no: "Nein", enquiry: "Anfrage vorbereiten", information: "Erst Leistung verstehen", quoteReview: "Vorhandenes Angebot prüfen", result: "Unverbindliche Orientierung", required: "Angaben für den nächsten Schritt", signature: "Passende besondere Lösung", addOn: "Mögliche Ergänzung", open: "Mehr erfahren", cta: "Anfrage vorbereiten", details: "Leistungsseite öffnen", restart: "Neu starten", privacy: "Alle Antworten bleiben ausschließlich in diesem Browser. Es wird nichts gespeichert oder übertragen, bevor Sie selbst einen Kontaktweg öffnen und Angaben absenden.", step: "Schritt", back: "Zurück", continue: "Weiter", show: "Empfehlung anzeigen",
  noMatchTitle: "Für diese Kombination ist keine bestätigte Leistung hinterlegt.",
  noMatchText: "Die Auswahl empfiehlt keine andere Region und erfindet keine Verfügbarkeit. Ändern Sie Ihre Angaben oder öffnen Sie die Leistungsübersicht.",
  photosReminder: "Fotos nach Möglichkeit ergänzen",
  selectedRequirements: "Berücksichtigte Anforderungen",
} as const;

const enCopy = {
  region: "Which region is the service for?", audience: "Is this a private or business enquiry?", category: "What is the main task?", route: "Where does the move start and end?", start: "Start location or postcode", destination: "Destination or postcode", startPlaceholder: "e.g. Regensburg", destinationPlaceholder: "e.g. Nuremberg", routeSummary: "Selected route", cadence: "One-off or recurring?", objectType: "Which property type fits best?", situation: "Which situation best describes the task?", requirements: "What needs particular attention?", hasOffer: "Do you already have a quote?", hasPhotos: "Are photos available?", nextStep: "What would you like to do next?", contactMethod: "How would you like to be contacted?", phone: "Phone", whatsapp: "WhatsApp", email: "Email", private: "Private", business: "Business", cleaning: "Cleaning", moving: "Moving or transport", clearance: "Clearance", offer: "Review a quote", oneOff: "One-off", recurring: "Recurring", standard: "Normally planned service", deadline: "Deadline or last-minute change", sensitive: "Sensitive or discreet situation", combined: "Combine several services", access: "Access, keys or alarm", handover: "Handover or move-out", coordination: "Coordinate several parties", none: "No special requirement", yes: "Yes", no: "No", enquiry: "Prepare an enquiry", information: "Understand the service first", quoteReview: "Review an existing quote", result: "Non-binding guidance", required: "Details for the next step", signature: "Relevant special solution", addOn: "Possible additional service", open: "Learn more", cta: "Prepare enquiry", details: "Open service page", restart: "Start again", privacy: "All answers remain in this browser. Nothing is stored or transmitted until you choose a contact route and submit details yourself.", step: "Step", back: "Back", continue: "Continue", show: "Show recommendation",
