...
const concerns = [
  {
    title: "Ich weiß nicht, welche Leistung passt.",
  ...
  },
  {
    ...
    answer: "Ort, grober Umfang und Fotos reichen oft für den ersten Schritt. Details können später folgen.",
    Icon: MessageCircle,
  },
  {
      ...
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-base leading-8 text-slate-700">
            Die wichtigsten Zweifel lassen sich ohne Druck klären. Eine Anfrage ist noch keine Buchung.
          </p>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {concerns.map((item) => (
            <article key={item.title} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-950/5">
              ...
              <p className="mt-2 text-sm leading-7 text-slate-700">{germanText(item.answer, item.answer)}</p>
            </article>
          ))}
        </div>
      </div>
