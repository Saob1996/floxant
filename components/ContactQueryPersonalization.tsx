...
function RequestContextSelector({
  ...
  const locationOptions: Array<{ value: RequestLocation; label: string; description: string }> = [
    ...requestLocationOptions.map((option) => ({
      value: option.id,
      label: option.label,
      description: `Leistung im Raum ${option.label}`,
    })),
    {
      value: "unsicher",
      label: "Anderer Ort",
      description: "Einsatzgebiet unverbindlich prüfen",
    },
  ];
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          {locationOptions.map((option) => (
            <button
              ...
              className={cn(
                "min-h-20 rounded-lg border px-4 py-3 text-left transition focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600",
            ...
            >
              ...
              <span className={cn("mt-1 block text-xs font-semibold leading-5", location === option.value ? "text-blue-100" : "text-slate-500")}>
                {option.description}
              </span>
            </button>
          ))}
