    ...
    <section className={`bg-slate-50 px-5 py-14 sm:px-8 lg:px-10 ${className}`}>
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[0.84fr_1.16fr]">
        <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-4 text-sm font-black uppercase tracking-normal text-blue-700">{germanizeText(eyebrow)}</p>
          <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950">{germanizeText(title)}</h2>
          {intro ? <p className="mt-4 text-sm font-semibold leading-7 text-slate-600">{germanizeText(intro)}</p> : null}
        </article>
