    ...
    <section className={`bg-slate-50 px-5 py-14 text-slate-950 sm:px-8 lg:px-10 ${className}`}>
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 text-base font-semibold leading-8 text-slate-700">{germanizeText(intro)}</p>
        </div>
        <div className="mt-8 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {problemBasedServiceLinks.slice(0, limit).map((link) => (
            ...
            >
              <h3 className="text-base font-black text-slate-950">{germanizeText(link.title)}</h3>
              <p className="mt-3 text-sm font-semibold leading-7 text-slate-700">{germanizeText(link.text)}</p>
              <span className="mt-4 inline-flex items-center gap-2 text-sm font-black text-blue-700">
                {germanizeText(link.cta || "Startpunkt öffnen")}
