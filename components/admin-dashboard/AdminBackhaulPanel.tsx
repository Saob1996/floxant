...
import {
  ...
  LEGACY_ADMIN_BACKHAUL_OFFER_SELECT,
...
export function AdminBackhaulPanel() {
  ...
  const [schemaMode, setSchemaMode] = useState<"operations" | "legacy">("operations");
  ...
  const loadOffers = useCallback(async () => {
    ...
    if (queryError?.code === "42703") {
      const { data: legacyData, error: legacyError } = await supabase
        .from("backhaul_offers")
        .select(LEGACY_ADMIN_BACKHAUL_OFFER_SELECT)
        .order("departure_date", { ascending: false });
      if (legacyError) {
        setOffers([]);
        setTableAvailable(legacyError.code !== "42P01" && legacyError.code !== "PGRST205");
        setError("Rückfahrten konnten nicht geladen werden. Admin-Rolle und Zugriffsregeln prüfen.");
      } else {
        setSchemaMode("legacy");
        setTableAvailable(true);
        setOffers((legacyData as unknown as LegacyBackhaulOfferRow[]).map(mapLegacyBackhaulOfferRow));
      }
    } else if (queryError) {
      setOffers([]);
  ...
  async function saveOffer(event: React.FormEvent<HTMLFormElement>) {
    ...
    const offer: BackhaulOffer = {
      id: editingId || crypto.randomUUID(),
      routeId: draft.routeId.trim(),
      title: draft.title.trim(),
      date: draft.date,
      dateEnd: draft.dateEnd || draft.date,
      timeWindow: draft.timeWindow.trim() || "nach Absprache",
      origin: draft.origin.trim(),
      destination: draft.destination.trim() || "Regensburg",
      intermediateStops: parseRouteAreas(draft.intermediateStops),
      pickupRadiusKm: optionalNumber(draft.pickupRadiusKm),
      destinationRadius: draft.destinationRadius.trim() || "ca. 200 km um Regensburg",
      routeAreas: parseRouteAreas(draft.routeAreas),
      vehicleType: draft.vehicleType.trim() || "Transporter oder LKW nach Tour",
      availableCapacity: draft.availableCapacity.trim(),
      availableCubicMeters: optionalNumber(draft.availableCubicMeters),
      loadingArea: draft.loadingArea.trim(),
      weightLimitKg: optionalNumber(draft.weightLimitKg),
      requiredHelpers: optionalNumber(draft.requiredHelpers),
      itemTypes: parseRouteAreas(draft.itemTypes),
      capacityMode: draft.capacityMode,
      internalNetPrice: optionalNumber(draft.internalNetPrice),
      publicGrossPrice: optionalNumber(draft.publicGrossPrice),
      vatRate: optionalNumber(draft.vatRate) ?? 19,
      priceType: draft.priceType,
      conditions: draft.conditions.trim(),
      publicDescription: draft.publicDescription.trim() || draft.fairPriceNote.trim(),
      publicationStatus: draft.publicationStatus,
      priceHint: draft.priceHint.trim() || "fairer Rückfahrt-Preis nach Route und Volumen",
      fairPriceNote: draft.fairPriceNote.trim(),
      status: draft.status,
    ...
    const payload = {
      ...
      date_end: offer.dateEnd,
      time_window: offer.timeWindow,
      origin: offer.origin,
      destination: offer.destination,
      intermediate_stops: offer.intermediateStops,
      pickup_radius_km: offer.pickupRadiusKm,
      ...
      available_cubic_meters: offer.availableCubicMeters,
      loading_area: offer.loadingArea,
      weight_limit_kg: offer.weightLimitKg,
      required_helpers: offer.requiredHelpers,
      item_types: offer.itemTypes,
      capacity_mode: offer.capacityMode,
      internal_net_price: offer.internalNetPrice,
      public_gross_price: offer.publicGrossPrice,
      vat_rate: offer.vatRate,
      price_type: offer.priceType,
      conditions: offer.conditions,
      public_description: offer.publicDescription,
      publication_status: offer.publicationStatus,
    ...
    const legacyPayload = {
      ...
      vehicle_type: offer.vehicleType,
      available_capacity: offer.availableCapacity,
      price_hint: offer.publicGrossPrice === null ? offer.priceHint : `${offer.publicGrossPrice.toLocaleString("de-DE")} € brutto · ${offer.priceHint}`,
      fair_price_note: offer.publicDescription || offer.fairPriceNote,
      status: toLegacyBackhaulStatus(offer.status, offer.publicationStatus),
      admin_note: encodeLegacyBackhaulMetadata(offer),
    };
  ...
  async function changeStatus(offer: BackhaulOffer, status: BackhaulOfferStatus, publish = false) {
    ...
    setError("");
    setSuccess("");
    const publicationStatus = publish ? "published" : offer.publicationStatus;
    const nextOffer = { ...offer, status, publicationStatus, updatedAt: new Date().toISOString() };
    const updatePayload = schemaMode === "operations"
      ? { status, ...(publish ? { publication_status: "published" } : {}) }
      : {
          status: toLegacyBackhaulStatus(status, publicationStatus),
          admin_note: encodeLegacyBackhaulMetadata(nextOffer),
        };
  ...
  return (
    <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1.1fr)_minmax(24rem,.9fr)]">
      <section className="min-w-0 rounded-2xl border border-white/10 bg-white/[0.045] shadow-[0_24px_80px_rgba(0,0,0,0.2)]">
        ...
        {tableAvailable && schemaMode === "legacy" ? (
          <div className="m-5 rounded-xl border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm font-semibold leading-6 text-cyan-50">
            Die Tourverwaltung läuft im kompatiblen Produktionsmodus. Alle Formularangaben werden geschützt gespeichert; öffentlich erscheinen ausschließlich aktiv veröffentlichte und noch gültige Fahrten.
          </div>
        ) : null}
