...
assert.equal(replies.getReplyTemplates("de").length, 12);
assert.equal(replies.getReplyTemplates("en").length, 12);
assert.equal(replies.adminReplyTemplates.length, 24);
...
const missingTemplate = replies.getRecommendedReplyTemplateKey(incompleteMoving.missing.length, "new");
assert.equal(missingTemplate, "missing_details");

console.log("Admin-Operations-Test erfolgreich: Vollständigkeitsregeln, 12 DE-/12 EN-Vorlagen und deterministische Entwürfe geprüft.");
