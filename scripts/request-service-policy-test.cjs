...
const locations = ["duesseldorf", "regensburg"];
const expectedServicesByCategory = {
  cleaning: [
    "reinigung",
...
const expectedServicesByLocation = {
  duesseldorf: [
    ...expectedServicesByCategory.cleaning.filter((id) => id !== "endreinigung"),
    ...expectedServicesByCategory.offer_check,
    "diskret-service",
    "private-client",
  ],
  regensburg: Object.values(expectedServicesByCategory).flat(),
};
const expectedUnsureServices = [
  "reinigung",
  "umzug",
  "fernumzug",
  "solarreinigung",
...
test("Policy enthält exakt den freigegebenen regionalen Servicevertrag", () => {
  assert.equal(
    projectedById.size,
    REQUEST_SERVICE_POLICY.length,
    "Service-IDs in der Anfrage-Policy müssen eindeutig sein",
  );
  const expectedIds = Object.values(expectedServicesByCategory).flat();
  assert.deepEqual(ids(REQUEST_SERVICE_POLICY), sorted(expectedIds));

  for (const [category, expectedIdsForCategory] of Object.entries(expectedServicesByCategory)) {
    assert.deepEqual(
      ids(REQUEST_SERVICE_POLICY.filter((service) => service.category === category)),
      sorted(expectedIdsForCategory),
      `${category}: freigegebene Service-IDs weichen ab`,
    );
  }

  for (const location of locations) {
    assert.deepEqual(
      ids(getRequestServicesForLocation(location)),
      sorted(expectedServicesByLocation[location]),
      `${location}: Servicevertrag weicht ab`,
    );
  }

  assert.deepEqual(ids(REQUEST_UNSURE_SERVICES), sorted(expectedUnsureServices));
  for (const service of REQUEST_SERVICE_POLICY) {
    assert.ok(service.name.trim(), `${service.id}: sichtbare Bezeichnung fehlt`);
    assert.ok(REQUEST_FORM_PROFILES[service.formProfile], `${service.id}: Formularprofil fehlt`);
    assert.ok(service.locations.length > 0, `${service.id}: Standort fehlt`);
    assert.ok(service.locations.every((location) => locations.includes(location)), `${service.id}: unbekannter Standort`);
  }
});

test("Jede freigegebene Kombination ist in Policy, Optionen und Resolver auswählbar", () => {
  for (const service of REQUEST_SERVICE_POLICY) {
    for (const location of service.locations) {
      assert.equal(isAllowedRequestCombination(location, service.id), true);
      assert.equal(getRequestService(location, service.id)?.id, service.id);
      assert.ok(
        requestServiceOptionsByLocation[location].some(
          (option) => option.registryServiceId === service.id,
        ),
        `${service.id} fehlt in den ${location}-Optionen`,
      );
      ...
      assert.equal(context.valid, true, `${location}/${service.id} muss gültig sein`);
      assert.equal(context.neutral, false);
      assert.equal(context.notice, "");
      assert.equal(context.serviceKey, service.id);
      assert.equal(context.location, location);
    }
  }
});

test("Kontextbezogene Navigations-CTAs verwenden nur gültige Policy-Kombinationen", () => {
  ...
  for (const item of requestItems) {
    ...
    if (!location) {
      assert.equal(context.valid, false, `${item.label || item.problem}: standortlose Vorauswahl darf nicht gültig sein`);
      assert.equal(context.neutral, true, `${item.label || item.problem}: standortlose Vorauswahl muss neutral bleiben`);
      assert.equal(context.serviceKey, "", `${item.label || item.problem}: standortlose Vorauswahl darf keinen Service festhalten`);
      continue;
    }
    assert.equal(context.valid, true, `${item.label || item.problem}: ungültiger Anfragekontext`);
    assert.equal(
      context.serviceKey,
      getRequestService(location, service)?.id,
      `${item.label || item.problem}: Service wird außerhalb der Policy umklassifiziert`,
    );
    assert.equal(context.location, location);
  }
});

test("Englische Service-Aliase normalisieren auf aktive Policy-IDs", () => {
  const aliases = [
...
test("Explizit freigegebene Solarservices bleiben getrennte aktive IDs", () => {
  for (const service of ["solarreinigung", "pv-anlagen-reinigung"]) {
    const context = resolveRequestContext({
      location: "duesseldorf",
      service,
      source: "seo",
    });
    assert.equal(context.valid, true, service);
    assert.equal(context.neutral, false, service);
    assert.equal(context.serviceKey, service, service);
  }

  assertFriendlyNeutralContext(
    { location: "duesseldorf", service: "solar-cleaning", source: "seo" },
    "nicht freigegebener Solar-Alias",
  );
});
...
test("Düsseldorf enthält keine Moving-Core-Services", () => {
  ...
  assert.deepEqual(
    ...
    sorted(expectedServicesByCategory.cleaning.filter((id) => id !== "endreinigung")),
  );
});

test("Regensburg enthält freigegebene Reinigung, Umzug, Transport und Räumung", () => {
...
test("Alle Formularprofile besitzen exakt die sichtbaren Mindestfelder", () => {
  const expectedCoreFields = {
    cleaning: ["cityOrZip", "objectType", "areaSize"],
...
test("Freie Ortsangaben bleiben im unsicheren Standortprofil erhalten", () => {
  const context = resolveRequestContext({
    city: "Unbekannter Standort",
    service: "reinigung",
    source: "contact-selector",
  });
  assert.equal(context.valid, true);
  assert.equal(context.neutral, false);
  assert.equal(context.location, "unsicher");
  assert.equal(context.locationHint, "Unbekannter Standort");
  assert.equal(context.serviceKey, "reinigung");
});
