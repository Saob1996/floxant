...
function recommendTarget(label, currentPage = "") {
  ...
  if (hasAny(normalized, ["diskreter service"])) {
    return { targetPage: "/private-client-service", supportingPages: ["/diskreter-umzug-trennung-scheidung", "/kontakt?service=diskret-service&intent=diskret-service&source=website"], problem: "Diskret-Intent rankt, aber Snippet muss zurueckhaltender und eindeutiger werden", action: "title_description_improve; faq_add; internal_links_add", service: "diskret-service", city: "", intent: intentLabels["diskret-service"] };
  }
  ...
  if (hasAny(normalized, ["praxisreinigung duesseldorf", "professionelle praxisreinigung duesseldorf"])) {
    return { targetPage: "/duesseldorf/praxisreinigung", supportingPages: ["/duesseldorf/reinigung", "/kontakt?service=praxisreinigung&city=duesseldorf&intent=praxisreinigung-duesseldorf&source=website"], problem: "Dusseldorf-Praxisreinigung ist kaufnah, Zielroute im Worktree aber kritisch", action: "manual_review; title_description_improve", service: "praxisreinigung", city: "duesseldorf", intent: intentLabels.praxisreinigung };
  }
...
${mdTable(
    ...
    [
      ["B2B Bueroreinigung", "/regensburg/bueroreinigung, /regensburg/gewerbereinigung", "Flaeche, Turnus, Raumliste und Zeitfenster klar beantworten", "Angebot pruefen, Randzeiten, Ansprechpartner", "/kontakt?service=bueroreinigung&intent=bueroreinigung-angebot-pruefen&source=website", "/regensburg/gewerbereinigung, /angebot-guenstiger-pruefen", "umgesetzt/weiter pruefen"],
      ["Praxisreinigung Dusseldorf", "/duesseldorf/praxisreinigung", "Hygienische Anforderungen vorsichtig erklaeren", "keine Zertifikate erfinden", "/kontakt?service=praxisreinigung&city=duesseldorf&intent=praxisreinigung-duesseldorf&source=website", "/duesseldorf/reinigung", "manuelle Routenpruefung wegen seo-gone"],
      ["Reinigung Dusseldorf", "/duesseldorf/reinigung", "Hub klar nach Service-Clustern strukturieren", "Fenster, Grundreinigung, Unterhalt, Angebot", "/kontakt?service=reinigung&city=duesseldorf&intent=reinigung-duesseldorf&source=website", "/duesseldorf/bueroreinigung, /duesseldorf/gewerbereinigung", "manuelle Routenpruefung"],
      ["Regensburg Umzug/Entruempelung", "/regensburg/umzug, /regensburg/entruempelung", "Primaerseiten gegen Legacy-Slugs staerken", "Angebot, Fotos, Etage, Zugang", "/kontakt?service=umzug&city=regensburg&intent=umzugsangebot-pruefen&source=website", "/umzug-regensburg, /angebot-guenstiger-pruefen", "umgesetzt/weiter pruefen"],
      ["Klaviertransport Regensburg", "/klaviertransport-regensburg", "Instrument, Etage, Zugang, Fotos, Termin", "Treppenhaus, Fluegel, Preis ohne Blindzusage", "/kontakt?service=klaviertransport&city=regensburg&intent=klaviertransport-regensburg&source=website", "/regensburg/umzug, /angebot-guenstiger-pruefen", "umgesetzt"],
      ["Diskret-Service", "/private-client-service", "Zurueckhaltende Einordnung sensibler Faelle", "Kontaktweg, keine Rechts-/Sicherheitsberatung", "/kontakt?service=diskret-service&intent=diskret-service&source=website", "/diskreter-umzug-trennung-scheidung, /kontakt", "umgesetzt"],
      ["Reinigung nach Entruempelung Landshut", "/entruempelung-landshut", "Besenrein vs. gruendlich nach Raeumung erklaeren", "Staub, Geruch, Uebergabe, Fotos", "/kontakt?service=entruempelung&city=landshut&intent=reinigung-nach-entruempelung-landshut&source=website", "/regensburg/reinigung, /angebot-guenstiger-pruefen", "umgesetzt ohne neue Seite"],
      ["Seniorenumzug / Umzug im Alter", "/seniorenumzug-bayern plus ausgewaehlte lokale Routen", "Angehoerige, Reduzierung, Uebergabe ruhig klaeren", "Packhilfe, Freigabe, Reinigung, Rueckruf", "/kontakt?service=umzug&intent=seniorenumzug&source=website", "/angebot-guenstiger-pruefen", "dynamische Routen pruefen"],
    ],
  )}
`;
