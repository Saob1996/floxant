...
const networkHubs = [
    { key: 'umzug-oberpfalz', name: 'Oberpfalz', type: 'Bezirk', desc: 'Die gesamte Oberpfalz von Regensburg bis Tirschenreuth.' },
    { key: 'umzug-landkreis-regensburg', name: 'Landkreis Regensburg', type: 'Landkreis', desc: 'Alle 41 Gemeinden und Märkte im direkten Umkreis der Domstadt.' }
];

function buildNetworkPage(hub) {
    const dir = path.join(appDir, hub.key);
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return generatePageSEO({
        lang,
        path: "${hub.key}",
        title: "Umzugsunternehmen ${hub.name} – Ihr regionaler Partner | FLOXANT",
        description: "Professioneller Transporte und Umzüge in ${hub.name}. ${hub.desc} 100% Versichert, Festpreisgarantie.",
    });
}

export default async function Hub${hub.key.replace(/-/g, '').charAt(0).toUpperCase() + hub.key.replace(/-/g, '').slice(1)}({ params }: { params: Promise<{ lang: string }> }) {
    const { lang } = await params;
    const dict = await getDictionary(lang as Locale);

    const bridgeLd = {
        "@context": "https://schema.org", "@type": "Service",
        "serviceType": "Umzug ${hub.name}",
        "provider": { "@type": "LocalBusiness", "name": "FLOXANT ${hub.name}" },
    ...
    return (
        <main className="min-h-screen bg-background">
            ...
            <Breadcrumbs lang={lang} items={[{ label: "Standorte", href: "/" + lang + "/service-area-bayern" }, { label: "${hub.name}" }]} />
            ...
            <section className="pt-20 pb-20 px-6 bg-muted/20">
                <div className="max-w-5xl mx-auto text-center space-y-8">
                    <h1 className="text-5xl md:text-6xl font-bold tracking-tight text-foreground">
                        Ihr Umzugspartner in der <span className="text-primary">${hub.name}</span>
                    </h1>
                    <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
                        Als tief in Ostbayern verwurzeltes Logistik- und Dienstleistungsunternehmen betreuen wir Privathaushalte und Firmenkunden zuverlässig im gesamten ${hub.type}.
                    </p>
                </div>
            </section>
            ...
            <section className="py-24 px-6 max-w-4xl mx-auto space-y-16">
                ...
                <div className="prose prose-lg max-w-none text-muted-foreground">
                    ...
                    <p>Ein Umzug über Gemeinde- oder Landkreisgrenzen hinweg erfordert ein Transportunternehmen, welches die lokalen Infrastrukturen kennt. ${hub.desc} Von ländlichen Resthöfen bis zu Etagenwohnungen im städtischen Ballungsraum – unsere Flotte und Teams sind für jedes Einsatzszenario in der ${hub.name} optimal ausgerüstet.</p>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="flex gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">Kurze Anfahrtswege, egal in welcher Kommune der ${hub.name} Sie starten oder ankommen.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-16 text-center">
                    <h2 className="text-3xl font-bold mb-8">Kostenfrei innerhalb der ${hub.name} anfragen</h2>
...
`;
    ...
    console.log(`Generated Bridge Hub: ${hub.name}`);
}

networkHubs.forEach(buildNetworkPage);
