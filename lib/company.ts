export const company = {
  name: "FLOXANT",
  email: "info@floxant.de",
  phone: "+49 1577 1105087",
  phoneRaw: "+4915771105087",
  address: "Johanna-Kinkel-Straße 1 + 2, 93049 Regensburg, Deutschland",
  streetAddress: "Johanna-Kinkel-Straße 1 + 2",
  postalCode: "93049",
  city: "Regensburg",
  state: "Bayern",
  country: "Deutschland",
  countryCode: "DE",
  vatId: "DE45971484",
  url: "https://www.floxant.de",
  bookingUrl: "https://www.floxant.de/buchung",
  contactUrl: "https://www.floxant.de/kontakt",
  locationsUrl: "https://www.floxant.de/standorte",
  serviceAreaUrl: "https://www.floxant.de/region-regensburg",
  businessProfilePreferredUrl: "https://www.floxant.de/buchung",
  mapsPreferredEntryUrl: "https://www.floxant.de/buchung",
  mapsSearchUrl:
    "https://www.google.com/maps/search/?api=1&query=Johanna-Kinkel-Stra%C3%9Fe%201%20%2B%202%2C%2093049%20Regensburg%2C%20Deutschland",
  geo: {
    lat: 49.0130057,
    lng: 12.074602,
  },
  sameAs: [
    "https://www.instagram.com/floxant_logistik",
    "https://www.facebook.com/floxant",
  ],
  primaryServiceAreas: [
    "Regensburg",
    "Gemeinden im verifizierten 75-km-Einsatzgebiet um Regensburg",
  ],
  keyGeoTargets: [
    "Regensburg",
    "Regensburg plus 75 km",
    "Neutraubling",
    "Lappersdorf",
    "Pentling",
    "Obertraubling",
    "Regenstauf",
    "Kelheim",
    "Schwandorf",
    "Straubing",
    "Bad Abbach",
  ],
  primaryCategories: [
    "Umzugsunternehmen",
    "Reinigungsfirma",
    "Entrümpelungsservice",
    "Büroumzug",
  ],
  coreServices: [
    "Umzug",
    "Reinigung",
    "Fassadenreinigung",
    "Notfallreinigung",
    "Entrümpelung",
    "Kellerentrümpelung",
    "Nachlassauflösung",
    "Büroumzug",
    "Möbeltransport",
    "Einlagerung",
    "Akteneinlagerung",
    "Firmenentsorgung",
    "Gewerbereinigung",
    "Übergabereinigung",
    "Endreinigung",
    "Plan-B-Service",
    "Private Client Service",
  ],
};

export const duesseldorfCompany = {
  name: "FLOXANT Düsseldorf",
  email: company.email,
  phone: company.phone,
  phoneRaw: company.phoneRaw,
  streetAddress: "Breite Str. 22",
  postalCode: "40213",
  city: "Düsseldorf",
  state: "Nordrhein-Westfalen",
  country: "Deutschland",
  countryCode: "DE",
  url: "https://www.floxant.de/duesseldorf",
  serviceAreaUrl: "https://www.floxant.de/duesseldorf/einsatzgebiet",
  geo: {
    lat: 51.2225767,
    lng: 6.7772364,
  },
};
