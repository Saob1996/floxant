...
export function resolveServiceFinder(input: ServiceFinderInput, locale: ServiceFinderLocale): ServiceFinderResult {
  ...
  if (input.region === "outside") {
    return {
      ...
      primaryCta: { label: isGerman ? "Region neutral anfragen" : "Ask about the region", href: isGerman ? "/kontakt?mode=neutral&intent=region-check" : "/en/contact" },
    };
  }
  ...
  if (input.region === "duesseldorf" && input.job !== "cleaning") {
    return {
      ...
      related: [],
      requiredDetails: requiredDetails(input.job, locale),
      primaryCta: { label: isGerman ? "Anliegen neutral schildern" : "Describe the request", href: isGerman ? "/kontakt?mode=neutral&intent=service-check" : "/en/contact" },
    };
  }
