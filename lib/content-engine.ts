...
export async function generateCityContent(city: string, service: string): Promise<GeneratedContent> {
 ...
 return {
  ...
  category: 'city_guide',
  htmlBlob,
  metadata: { city: displayCity, service, target_keywords: [`${germanizeText(normalizedService)} ${displayCity} kosten`, `${germanizeText(normalizedService)} ${displayCity} preis`] }
 };
}
