...
Stand: 13. Juli 2026. Diese Anleitung enthält keine Zugangsdaten oder echten Schlüssel.
...
Aktuell vorhandene Anfragearten sind `b2b_reinigung`, `budget_inquiry_quick`, `reinigung` und `umzug`. Das Formularsystem normalisiert außerdem weitere Leistungen und speichert sie in derselben Spalte `service`. Herkunft und Formulartyp werden innerhalb von `details`, insbesondere unter `details.service.source`, `details.service.entryPoint`, `details.metadata.source` und den strukturierten Konfigurationsfeldern gespeichert.

Die aktuell vorhandenen Statuswerte sind `new` und der historische Altwert `deleted`. Das neue Dashboard ändert ausschließlich die bestehende Spalte `status` und verwendet dafür `new` (Neu), `in_bearbeitung` (In Bearbeitung) und `erledigt` (Erledigt). Es gibt keine Löschfunktion.
...
## Sicherheitsbefund vor der Migration

Der öffentliche Anon-Key konnte am 13. Juli 2026 alle 27 vorhandenen `bookings`-Zeilen zählen. Damit besteht aktuell kein wirksamer anonymer Leseschutz; aus der REST-Schnittstelle allein lässt sich nicht unterscheiden, ob RLS deaktiviert ist oder eine zu breite SELECT-Policy besteht. Für den sicheren Betrieb muss die unten genannte Migration vor der Freigabe des Dashboards angewendet und anschließend getestet werden.
...
Die bestehende Formular-Function benötigt zusätzlich weiterhin ihre bereits vorhandenen serverseitigen Cloudflare-Variablen (`SUPABASE_URL` beziehungsweise als URL-Fallback `NEXT_PUBLIC_SUPABASE_URL` sowie `SUPABASE_SERVICE_ROLE_KEY`). `SUPABASE_SERVICE_ROLE_KEY`, Datenbankpasswörter, Resend-Schlüssel und private Tokens dürfen niemals als `NEXT_PUBLIC_*`, in Git oder im Dashboard-Code gespeichert werden.
...
Die Migration wurde in diesem Arbeitsschritt nicht ausgeführt. Vor dem Ausführen vorhandene Policies und Grants sichern:
...
Danach die Migrationsdatei im Supabase SQL Editor ausführen oder in einem korrekt verknüpften lokalen Supabase-Projekt über den üblichen Migrationsprozess anwenden. Die Migration:
...
- gibt kein Browser-`INSERT` und kein `DELETE` frei,
...
7. Mit Admin-JWT `DELETE` versuchen: muss scheitern.
8. Mit Anon-Key `bookings?select=id` aufrufen: darf keine Anfrage zurückgeben.
9. Öffentlichen Formularweg einmal ohne Upload und einmal mit erlaubtem Upload testen; neue Zeile muss weiterhin über die Cloudflare Function entstehen.
10. Browser-Bundle und Repository nach `SUPABASE_SERVICE_ROLE_KEY`, Datenbankpasswort und echten privaten Tokens durchsuchen.
...
2. Bestehende Function-Secrets unverändert lassen; insbesondere `SUPABASE_SERVICE_ROLE_KEY` nur als Secret für die Pages Functions führen.
3. Einen neuen statischen Build erst nach angewendeter RLS-Migration starten.
4. Build-Ausgabe `out` und bestehende Cloudflare-Pages-Einstellungen beibehalten.
5. Keine DNS-Änderung ist für das Dashboard erforderlich.
