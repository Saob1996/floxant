...
function normalizeService(service: unknown) {
 ...
 if (
  raw === "regensburg_moeblierte_wohnung_reinigung" ||
  raw === "regensburg_cleaning" ||
  raw === "regensburg_cleaning" ||
  raw === "regensburg_reinigung" ||
  raw === "regensburg_cleaning" ||
  raw === "regensburg_reinigung"
 ) {
...
function isRegensburgApartmentCleaningRequestType(value: unknown) {
 const normalized = String(value || "").trim().toLowerCase();
 return [
  "regensburg_cleaning",
  "regensburg_moeblierte_wohnung_reinigung",
  "regensburg-moeblierte-wohnung-reinigung",
  "regensburg_cleaning",
...
function isRegensburgB2BCleaningRequestType(value: unknown) {
 const normalized = String(value || "").trim().toLowerCase();
 return [
  "regensburg_b2b_cleaning",
  "regensburg_b2b_reinigung",
  "regensburg-b2b-reinigung",
  "regensburg_bueroreinigung",
  "regensburg-bueroreinigung",
  "regensburg_b2b_reinigung",
  "regensburg_bueroreinigung",
  "regensburg_gewerbereinigung",
  "regensburg_firmenreinigung",
 ].includes(normalized);
}
...
function resolveOfferCheckService(payload: any) {
 ...
 if (region === "regensburg" && !["reinigung", "b2b_reinigung", "entsorgung", "firmenentsorgung"].includes(normalizedService)) {
  return "reinigung";
 }
...
function buildRegensburgB2BCleaningDetails(
 payload: any,
 uploadMetadata: OfferCheckUploadMetadata[] = [],
): IntakePayload {
 ...
 const photoFiles = uploadMetadata.filter((item) => item.category === "photo");
 const createdAt = payloadText(payload, "timestamp") || new Date().toISOString();
 const source = payloadText(payload, "leadSource") || payloadText(payload, "source") || "regensburg_b2b_cleaning";
 const sourceComponent = payloadText(payload, "sourceComponent") || "regensburg_b2b_cleaning_form";
 ...
 return {
  ...
  valuation: {
   ...
   priceRangeMin: 0,
   priceRangeMax: 0,
   valuationLabel: "Regensburg B2B-Reinigung",
   valuationStage: "Objekt pruefen",
   accuracyState: "B2B-Reinigung Regensburg zur Vorpruefung",
   topDrivers: [
   ...
   priceSuggestion: budget,
   priceExplanation:
    "Regensburg-B2B-Reinigung: Preis und Machbarkeit werden nach Objektart, Flaeche, Frequenz, Zeitfenster, Zugang, Fotos und gewuenschter Leistung geprueft. Keine medizinische Spezialdesinfektion, keine 24/7- oder Kapazitaetsgarantie; Umzug laeuft ueber /regensburg/umzug.",
   pricingSignals: {
    inquiryMode: "regensburg_b2b_cleaning",
    leadType: "regensburg_reinigung",
    leadSubtype: "b2b",
    companyName,
    roleType,
    objectType,
  ...
  configuration: {
   requestContext: "regensburg_b2b_cleaning",
   leadType: "regensburg_reinigung",
   leadSubtype: "b2b",
   sourceComponent,
   leadSource: source,
   ...
   scopeSummary: customerMessage,
   legalNote:
    "Regensburg hat bei FLOXANT eigene Kontaktwege. Diese Anfrage betrifft B2B-Reinigung fuer kleine Unternehmen nach Absprache; keine medizinische Spezialreinigung, keine Hygienezertifizierung und keine 24/7-Zusage.",
  },
  metadata: {
   createdAt,
   intakeVersion: "2.4.0",
   ...
   clientContext: {
    ...
    sourceComponent,
    entryPoint: "/regensburg/bueroreinigung",
    landingPage: payloadText(payload, "landingPage") || "/regensburg/bueroreinigung",
    referrer: payloadText(payload, "referrer"),
    utmSource: payloadText(payload, "utmSource"),
    utmMedium: payloadText(payload, "utmMedium"),
    utmCampaign: payloadText(payload, "utmCampaign"),
...
function buildRegensburgApartmentCleaningDetails(
 payload: any,
 uploadMetadata: OfferCheckUploadMetadata[] = [],
): IntakePayload {
 const normalizedService = "regensburg_moeblierte_wohnung_reinigung";
 const roleType = payloadText(payload, "roleType") || payloadText(payload, "role_type");
 const objectType = payloadText(payload, "objectType") || payloadText(payload, "object_type");
 const objectLocation =
  payloadText(payload, "objectLocation") ||
 ...
 const photoFiles = uploadMetadata.filter((item) => item.category === "photo");
 const createdAt = payloadText(payload, "timestamp") || new Date().toISOString();
 const source = payloadText(payload, "leadSource") || payloadText(payload, "source") || "regensburg_cleaning";
 const normalizedText = [
  roleType,
  objectType,
  cleaningType,
 ...
 return {
  ...
  service: {
   type: normalizedService,
   source,
   entryPoint: "/regensburg/reinigung",
   presetFromUrl: normalizedService,
   regionPreset: "duesseldorf",
  },
  valuation: {
   ...
   priceSuggestion: budget,
   priceExplanation:
    "Regensburg-Apartment-Reinigung: Preis und Machbarkeit werden nach Objektart, Terminfenster, Zustand, Fotos, Zugang und Zusatzwuenschen geprueft. Keine Airbnb-Partnerschaft, keine Hotelstandard-Garantie, Waesche/Schluessel/Inventar nur nach Absprache.",
   pricingSignals: {
    inquiryMode: "regensburg_cleaning",
  ...
  configuration: {
   requestContext: "regensburg_cleaning",
   leadType: normalizedService,
   entryPoint: "/regensburg/reinigung",
   region: "duesseldorf",
   regionPreset: "duesseldorf",
   ...
   scopeSummary: customerMessage,
   legalNote:
    "Reinigung fuer moeblierte Wohnungen in Regensburg nach Absprache; keine Airbnb-Partnerschaft, keine Hotelstandard-Garantie, Umzug separat ueber /regensburg/umzug, keine Schluessel-/Waesche-/Inventar-Garantie.",
  },
  metadata: {
   createdAt,
   intakeVersion: "2.3.0",
   ...
   clientContext: {
    ...
    leadSource: source,
    entryPoint: "/regensburg/reinigung",
    landingPage: payloadText(payload, "landingPage") || "/regensburg/reinigung",
    referrer: payloadText(payload, "referrer"),
    utmSource: payloadText(payload, "utmSource"),
    utmMedium: payloadText(payload, "utmMedium"),
    utmCampaign: payloadText(payload, "utmCampaign"),
...
export async function POST(req: Request) {
 ...
 try {
  ...
  if (contentType.includes("application/json")) {
   ...
   const isRegensburgB2BCleaning = isRegensburgB2BCleaningRequestType(requestType);
   const isRegensburgApartmentCleaning = isRegensburgApartmentCleaningRequestType(requestType);
   ...
   fileUrls = [];
   } else if (isRegensburgB2BCleaning) {
    ...
    offerUploadMetadata.push(...(await uploadOfferCheckFiles(photoFiles, "photo", "regensburg-b2b-cleaning")));
    fileUrls = [];
   } else if (isRegensburgApartmentCleaning) {
    ...
    offerUploadMetadata.push(...(await uploadOfferCheckFiles(photoFiles, "photo", "regensburg-apartment-cleaning")));
    fileUrls = [];
   } else {
   ...
   payload = {
    ...
    missingInfoFlags: formData.get("missingInfoFlags"),
    hasPhotos: formData.get("hasPhotos"),
    hasOffer: formData.get("hasOffer"),
    signatureServiceHint: formData.get("signatureServiceHint"),
    leadPriority: formData.get("leadPriority"),
    responseTemplateKey: formData.get("responseTemplateKey"),
    recommendedNextStep: formData.get("recommendedNextStep"),
    missingInfoQuestions: formData.get("missingInfoQuestions"),
  ...
  const rawDetails =
   payload.type === "offer_check" || isOfferCheckRequestType(payload.lead_type)
    ...
    : payload.type === "regensburg_b2b_cleaning" || isRegensburgB2BCleaningRequestType(payload.lead_type)
     ? buildRegensburgB2BCleaningDetails(payload, payload.offerUploadMetadata || [])
    : payload.type === "regensburg_cleaning" || isRegensburgApartmentCleaningRequestType(payload.lead_type)
     ? buildRegensburgApartmentCleaningDetails(payload, payload.offerUploadMetadata || [])
