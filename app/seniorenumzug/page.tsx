...
import { Heart, Shield, Clock, Star, Zap, Truck } from "lucide-react";
interface PageProps {
  params: Promise<{}>;
}
...
export default async function SeniorenumzugPage({ params }: PageProps) {
  const locale = "de";
  const { 
    localeDict, 
    content, 
  ...
  return (
    <SpecialtyPageLayout
      pagePath="/regensburg/seniorenumzug"
        lang="de"
        dict={localeDict}
        city={city}
        ...
        chips={[
          { icon: Heart, text: resolveNestedField(content.badges, fallback.badges, "permit", city) },
