...
import { HardDrive, Shield, Clock, Star, Zap, Archive } from "lucide-react";
interface PageProps {
  params: Promise<{}>;
}
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { seoContent, seoFallback, city } = await getSpecialtyPageData({
    locale: "de",
    baseKey: "einlagerung_spec",
    city: "Bayern",
  });
  return generatePageSEO({
    lang: "de",
    path: `einlagerung`,
    title: "Einlagerung Bayern | Möbel und Hausrat lagern | FLOXANT",
...
export default async function EinlagerungPage({ params }: PageProps) {
  ...
  return (
    <SpecialtyPageLayout
        ...
        chips={[
          { icon: Archive, text: resolveNestedField(content.badges, fallback.badges, "permit", city) },
