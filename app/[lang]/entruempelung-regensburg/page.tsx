...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const safeTitle = (content.meta_title || "").replace('{city}', 'Regensburg');
    const safeDesc = (content.meta_desc || "").replace('{city}', 'Regensburg');
...
export default async function GenericServicePage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const serviceName = (content.hero_h1 || "Service") + " Regensburg";
    const serviceDesc = (content.meta_desc || "").replace('{city}', 'Regensburg');
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": serviceName, "item": "https://www.floxant.de/" + pageLocale + "/entruempelung-regensburg" }
        ]
    };
    ...
    return (
        <main className="min-h-screen bg-background">
            ...
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: content.link_umzug?.replace('{city}', 'Regensburg') || "Umzug Regensburg", href: "/" + pageLocale + "/umzug-regensburg" }, { label: content.hero_h1 || "Service" }]} />
            ...
            <section className="py-24 px-6 text-start">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="prose prose-xl max-w-none text-muted-foreground leading-loose">
                        <h2 className="text-3xl font-extrabold text-foreground">{(content.section2_h2 || "").replace('{city}', 'Regensburg')}</h2>
                        <p>{(content.section2_p1 || "").replace('{city}', 'Regensburg')}</p>
                        <p>{(content.section2_p2 || "").replace('{city}', 'Regensburg')}</p>
                    </div>
                    ...
                    <div id="wizard" className="text-center py-16 bg-card rounded-[3rem] border border-border shadow-2xl relative mt-16 scroll-mt-24">
                        ...
                        <h2 className="text-4xl font-extrabold mb-6 mt-6">{(content.wizard_h2 || "").replace('{city}', 'Regensburg')}</h2>
