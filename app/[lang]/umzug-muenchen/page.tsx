...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return generatePageSEO({
        pageLocale,
        path: 'umzug-muenchen',
        title: 'Umzugsunternehmen in München | FLOXANT',
        description: 'Professionelle Umzugsunternehmen in München in Bayern. Seriöse Abwicklung, Festpreisgarantie und versicherter Transport. Jetzt online berechnen!',
    });
}
...
export default async function UmzugMuenchen({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in München?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Ein lokaler Umzug in München kostet zwischen 400 und 2.000 Euro je nach Wohnungsgröße. FLOXANT bietet verbindliche Festpreise nach kostenloser Besichtigung." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Macht FLOXANT auch Umzüge in der Altstadt von München?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Ja, wir sind auf schwierige Logistik wie enge Gassen oder viele Treppen spezialisiert und kümmern uns um alle Halteverbotszonen." } },
                { "@type": "Question", "name": content.faqs?.[2]?.q || "Bieten Sie Fernumzüge ab München an?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[2]?.a || "Ja. Wir organisieren Fernumzüge von München nach ganz Deutschland." } }
            ],
    };
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug München", "item": "https://www.floxant.de/" + pageLocale + "/umzug-muenchen" }
        ]
    };

    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Bayern", href: "/" + pageLocale + "/umzug-bayern" }, { label: "Umzug München" }]} />
            ...
            <section className="py-20 px-6">
                <div className="max-w-4xl mx-auto space-y-24">
                    ...
                    <div className="prose prose-lg max-w-none text-muted-foreground">
                        ...
                        <p>Wer nach München zieht, oder die Stadt verlässt, muss oft auch alte Möbel entsorgen. FLOXANT bietet exzellente <Link href={"/" + pageLocale + "/entruempelung"}>{dict.common.entruempelung_services}</Link>{dict.common.waste_disposal_desc}</p>
