...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
    ...
    return generatePageSEO({
        ...
        description: "Warum FLOXANT die bessere Wahl für Ihren Umzug in Bayern ist. Transparente Kosten, echte Festpreise und intelligente Logistik im Vergleich.",
    });
}

export default async function AlternativenLandingPage({ params }: { params: Promise<{ lang: string }> }) {
    const { lang } = await params;
        ...
        <main className="min-h-screen bg-[#05050A] text-white pt-32 pb-24">
            <div className="max-w-4xl mx-auto px-4 text-center">
                ...
                <p className="text-xl text-white/60 mb-12 leading-relaxed">
                    Suchen Sie nach anderen Anbietern? Vergleichen Sie unsere Leistungen und Preise, bevor Sie sich entscheiden. Wir setzen auf 100% Transparenz und Festpreisgarantie.
                </p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-start">
                    {[
                        { title: "Günstiger als Mitbewerber", slug: "anbieter-regensburg", desc: "Wie unsere Algorithmen Kosten sparen, die wir direkt an Sie weitergeben." },
                        { title: "Service im Vergleich", slug: "umzugsfirma-bayern", desc: "Warum unser 3-Tier Tarifsystem flexibler ist als statische Pauschalen." },
                    ].map((item) => (
                        <Link 
                            key={item.slug} 
                            href={`/${lang}/alternativen/${item.slug}`}
