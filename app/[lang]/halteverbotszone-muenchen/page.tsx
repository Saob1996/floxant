...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.halteverbotszone_muenchen || {};
    return generatePageSEO({
        pageLocale,
        path: "halteverbotszone-muenchen",
        title: content.meta_title || "Halteverbotszone München – Beantragung & Aufstellu | FLOXANT",
        description: content.meta_desc || `Halteverbotszone in München beantragen: Wir übernehmen die behördliche Genehmigung und stellen die Schilder auf. Rechtskonform, fristgerecht, stressfrei. Sofortpreis online berechnen oder bequem per WhatsApp / Telefon anfragen: +49 1577 1105087.`,
    });
}
...
export default async function HalteverbotPage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug München", href: "/" + pageLocale + "/umzug-muenchen" }, { label: "Halteverbotszone München" }]} />
            ...
            <section className="pt-12 pb-24 px-6 bg-gradient-to-b from-amber-50 dark:from-amber-950/20 via-muted/30 to-background overflow-hidden relative">
                <div className="max-w-7xl mx-auto text-center space-y-8 relative z-10">
                    ...
                    <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-foreground leading-tight">
                        ...
                        <span className="text-amber-600 dark:text-amber-400">München</span>
                    </h1>
                    <p className="text-xl md:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed font-medium">
                        Kein Parkplatzstress am Umzugstag. Wir beantragen, genehmigen und beschildern Ihre Halteverbotszone in München – rechtskonform und fristgerecht.
                    </p>
            ...
            <section className="py-24 px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                        {[
                            { step: "1", title: "Antrag stellen", desc: "Wir beantragen die Halteverbotszone bei der zuständigen Behörde in München. Vorlaufzeit: min. 14 Tage." },
                    ...
                    <div className="prose prose-xl max-w-none text-muted-foreground leading-loose">
                        <h2 className="text-3xl font-extrabold text-foreground">Halteverbotszone in München: Was Sie wissen müssen</h2>
                        <p>In München ist die Halteverbotszone über die städtische Straßenverkehrsbehörde zu beantragen. Je nach Standort (Innenstadt wie Schwabing vs. Außenbezirk) variieren Bearbeitungszeiten und Gebühren.</p>
                        ...
                        <p><strong>Tipp:</strong> In engeren Straßen von  Bogenhausen empfehlen wir, die Halteverbotszone beidseitig einzurichten, um genügend Rangierraum für den Umzugs-LKW zu schaffen.</p>
                    </div>
