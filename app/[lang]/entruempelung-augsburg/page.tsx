...
import { Truck, Shield, Briefcase, Star, Clock, Hammer } from "lucide-react";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const { seoContent, seoFallback, city } = await getSpecialtyPageData({
        ...
        seoKey: "entruempelung_augsburg", // Use city-specific SEO key
        city: "Augsburg",
    });
        ...
        "@graph": [
            ...
            },
            {
                ...
                "itemListElement": [
                    ...
                    { "@type": "ListItem", "position": 3, "name": `Augsburg`, "item": `https://www.floxant.de/${lang}/entruempelung-augsburg` }
                ]
            },
            ...(faqItems.length > 0 ? [{
                "@type": "FAQPage",
    ...
    return (
        <>
            ...
            <SpecialtyPageLayout
                ...
                chips={[
                    { icon: Briefcase, text: resolveNestedField(content.badges, fallback.badges, "permit", city) },
                    { icon: Truck, text: resolveNestedField(content.badges, fallback.badges, "signs", city) },
                    { icon: Shield, text: resolveNestedField(content.badges, fallback.badges, "stressfree", city) }
                ]}
                cards={[
                    {
                        icon: Star,
                    ...
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {[
                                ...
                                { name: "Rosenheim", href: "/de/entruempelung-rosenheim" },
                                { name: "Straubing", href: "/de/entruempelung-straubing" },
                                { name: "Passau", href: "/de/entruempelung-passau" },
                                { name: "Würzburg", href: "/de/entruempelung-wuerzburg" },
                                { name: "Deggendorf", href: "/de/entruempelung-deggendorf" }
                            ].map((loc) => (
                                <Link 
                                    key={loc.name} 
                                    href={loc.href}
