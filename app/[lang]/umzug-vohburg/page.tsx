...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.umzug_vohburg || {};
    return generatePageSEO({
        pageLocale,
        path: "umzug-vohburg",
        title: content.meta_title || "Umzugsunternehmen Vohburg an der Donau | FLOXANT",
        description: 'Ihr Umzugsunternehmen für Vohburg an der Donau (Oberbayern). Professioneller Umzug mit $..., vol...',
    });
}
...
export default async function UmzugVohburg({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in Vohburg an der Donau?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Die Kosten hängen von Wohnungsgröße, Etage und Entfernung ab. Ein Studentenumzug startet bei günstigen Einstiegspreisen. Familien erhalten ein maßgeschneidertes Festpreisangebot nach kostenloser Besichtigung." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Wie schnell ist FLOXANT in Vohburg an der Donau verfügbar?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Da unsere Teams täglich auf der Strecke A9 Ingolstadt–München unterwegs sind, können wir oft schon innerhalb weniger Tage einen Termin in Vohburg an der Donau realisieren." } },
                ...
                { "@type": "Question", "name": content.faqs?.[3]?.q || "Bieten Sie auch Entrümpelung in Vohburg an der Donau an?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[3]?.a || "Ja. Wir kombinieren Umzug, Entrümpelung und Endreinigung zu einem Paket. Ideal bei Wohnungsauflösungen oder wenn die alte Wohnung besenrein übergeben werden muss." } }
            ],
    };
    ...
    const localBusinessJsonLd = {
        ...
        "url": "https://www.floxant.de/" + pageLocale + "/umzug-vohburg",
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Vohburg an der Donau", "item": "https://www.floxant.de/" + pageLocale + "/umzug-vohburg" }
        ]
    };

    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Bayern", href: "/" + pageLocale + "/umzug-bayern" }, { label: "Umzug Vohburg an der Donau" }]} />
