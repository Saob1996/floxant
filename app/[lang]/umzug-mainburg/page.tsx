...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.umzug_mainburg || {};
    return generatePageSEO({
        pageLocale,
        path: "umzug-mainburg",
        title: content.meta_title || "Umzugsunternehmen Mainburg | FLOXANT",
        description: 'Ihr Umzugsunternehmen für Mainburg (Niederbayern). Professioneller Umzug mit $..., voller Versic...',
    });
}
...
export default async function UmzugMainburg({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in Mainburg?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Die Kosten hängen von Wohnungsgröße, Etage und Entfernung ab. Ein Studentenumzug startet bei günstigen Einstiegspreisen. Familien erhalten ein maßgeschneidertes Festpreisangebot nach kostenloser Besichtigung." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Wie schnell ist FLOXANT in Mainburg verfügbar?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Da unsere Teams täglich auf der Strecke A93 Regensburg–München unterwegs sind, können wir oft schon innerhalb weniger Tage einen Termin in Mainburg realisieren." } },
                ...
                { "@type": "Question", "name": content.faqs?.[3]?.q || "Bieten Sie auch Entrümpelung in Mainburg an?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[3]?.a || "Ja. Wir kombinieren Umzug, Entrümpelung und Endreinigung zu einem Paket. Ideal bei Wohnungsauflösungen oder wenn die alte Wohnung besenrein übergeben werden muss." } }
            ],
    };
    ...
    const localBusinessJsonLd = {
        ...
        "url": "https://www.floxant.de/" + pageLocale + "/umzug-mainburg",
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Mainburg", "item": "https://www.floxant.de/" + pageLocale + "/umzug-mainburg" }
        ]
    };

    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Bayern", href: "/" + pageLocale + "/umzug-bayern" }, { label: "Umzug Mainburg" }]} />
