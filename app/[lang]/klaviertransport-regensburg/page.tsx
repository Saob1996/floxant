...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.klaviertransport_regensburg || {};
    return generatePageSEO({
        pageLocale,
        path: "klaviertransport-regensburg",
        title: content.meta_title || "Klaviertransport & Tresortransport Regensburg | FL | FLOXANT",
        description: content.meta_desc || `Klaviertransport, Flügeltransport und Tresortransport in Regensburg. Spezialisiertes Equipment, geschultes Team, volle Versicherung. Jetzt anfragen! Sofortpreis online berechnen oder bequem per WhatsApp / Telefon anfragen: +49 1577 1105087.`,
    });
}
...
export default async function KlaviertransportPage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const serviceJsonLd = {
        ...
        "name": "Klaviertransport & Tresortransport Regensburg",
        "provider": {
    ...
    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Regensburg", href: "/" + pageLocale + "/umzug-regensburg" }, { label: "Klavier- & Tresortransport" }]} />
            ...
            <section className="pt-12 pb-24 px-6 bg-gradient-to-b from-slate-100 dark:from-slate-900/40 via-muted/30 to-background overflow-hidden relative">
                <div className="max-w-7xl mx-auto text-center space-y-8 relative z-10">
                    ...
                    <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-foreground leading-tight">
                        ...
                        <span className="text-primary">Regensburg</span>
                    </h1>
            ...
            <section className="py-24 px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="prose prose-xl max-w-none text-muted-foreground leading-loose">
                        <h2 className="text-3xl font-extrabold text-foreground">Schwertransporte in Regensburg: lokales Können</h2>
                        <p>In Regensburg erleben wir häufig Klaviertransporte aus Altbauwohnungen in Altstadt oder  Stadtamhof. Die Herausforderung: enge Treppenhäuser, keine Aufzüge, Wendeltreppen. Unsere Lösung: Außenlift oder Krantransport über den Balkon – je nach Situation.</p>
                        <p>Tresortransporte sind dagegen oft Firmenkunden-Aufträge. Wir transportieren Waffenschränke, Datensafes und Dokumententresore sowohl innerhalb von Regensburg als auch überregional. Dabei garantieren wir absolute Diskretion und volle Versicherungsdeckung.</p>
                    </div>
                    ...
                    <div id="wizard" className="text-center py-16 bg-card rounded-[3rem] border border-border shadow-2xl relative mt-16 scroll-mt-24">
                        ...
                        <h2 className="text-4xl font-extrabold mb-6 mt-6">Schwertransport in Regensburg anfragen</h2>
                    ...
                    <div className="flex flex-wrap justify-center gap-3 mt-8">
                        <Link href={"/" + pageLocale + "/umzug-regensburg"} className="px-5 py-3 rounded-2xl border border-border/50 bg-card text-sm font-semibold hover:border-primary/50 transition-all">Umzug Regensburg</Link>
                        <Link href={"/" + pageLocale + "/seniorenumzug-regensburg"} className="px-5 py-3 rounded-2xl border border-border/50 bg-card text-sm font-semibold hover:border-primary/50 transition-all">Seniorenumzug Regensburg</Link>
                        <Link href={"/" + pageLocale + "/halteverbotszone-regensburg"} className="px-5 py-3 rounded-2xl border border-border/50 bg-card text-sm font-semibold hover:border-primary/50 transition-all">Halteverbotszone Regensburg</Link>
                    </div>
                </div>
            </section>
        </main>
