...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.umzug_moetzing || {};
    return generatePageSEO({
        pageLocale,
        path: "umzug-moetzing",
        title: content.meta_title || "Umzug in Mötzing ab 79€ – Sofort verfügbar | FLOXANT",
        description: 'Professionelles Umzugsunternehmen für Mötzing. ✓ $... ✓ $.... Sofort anfragen!',
    });
}
...
export default async function UmzugMoetzing({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in Mötzing?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Ein Transporter mit Helfern ist oft schon ab günstigen Einstiegspreisen verfügbar. Für den durchschnittlichen Familienhaushalt kalkulieren wir maßgeschneiderte Festpreise, in der Regel zwischen 400€ und 1.500€, abhängig von Raumanzahl und Transportstrecke." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Wie kurzfristig ist ein Umzugstermin in Mötzing verfügbar?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Da sich unsere Disposition im nahen Regensburg befindet, können wir oft auch sehr kurzfristige Umzüge oder Notfall-Termine in Mötzing realisieren." } },
                { "@type": "Question", "name": content.faqs?.[2]?.q || "Kümmern Sie sich um Halteverbotszonen in Mötzing?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[2]?.a || "Ja, absolut. Wenn Parkraum vor Ihrer Haustür knapp ist, organisieren wir die offizielle Beantragung sowie das Aufstellen der Halteverbotsschilder durch die lokalen Behörden." } },
                { "@type": "Question", "name": content.faqs?.[3]?.q || "Bieten Sie auch Küchenmontage in Mötzing an?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[3]?.a || "Wir demontieren Ihre Küche fachmännisch und verpacken die Teile sicher. Den fachgerechten Wiederaufbau übernehmen wir im Rahmen der Machbarkeiten ebenfalls gerne." } },
    ...
    const localBusinessJsonLd = {
        ...
        "url": "https://www.floxant.de/" + pageLocale + "/umzug-moetzing",
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Mötzing", "item": "https://www.floxant.de/" + pageLocale + "/umzug-moetzing" }
        ]
    };

    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Region Regensburg", href: "/" + pageLocale + "/umzug-regensburg" }, { label: "Umzug Mötzing" }]} />
