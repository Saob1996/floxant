...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.halteverbotszone_regensburg || {};
    return generatePageSEO({
        pageLocale,
        path: "halteverbotszone-regensburg",
        title: content.meta_title || "Halteverbotszone Regensburg – Beantragung & Aufste | FLOXANT",
        description: content.meta_desc || `Halteverbotszone in Regensburg beantragen: Wir übernehmen die behördliche Genehmigung und stellen die Schilder auf. Rechtskonform, fristgerecht, stressfrei. Sofortpreis online berechnen oder bequem per WhatsApp / Telefon anfragen: +49 1577 1105087.`,
    });
}
...
export default async function HalteverbotPage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Regensburg", href: "/" + pageLocale + "/umzug-regensburg" }, { label: "Halteverbotszone Regensburg" }]} />
