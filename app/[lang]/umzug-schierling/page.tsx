...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.umzug_schierling || {};
    return generatePageSEO({
        pageLocale,
        path: "umzug-schierling",
        title: content.meta_title || "Umzug in Schierling ab 79€ – Sofort verfügbar | FLOXANT",
        description: 'Professionelles Umzugsunternehmen für Schierling. ✓ $... ✓ $.... Sofort anfragen!',
    });
}
...
export default async function UmzugSchierling({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in Schierling?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Ein Transporter mit Helfern ist oft schon ab günstigen Einstiegspreisen verfügbar. Für den durchschnittlichen Familienhaushalt kalkulieren wir maßgeschneiderte Festpreise, in der Regel zwischen 400€ und 1.500€, abhängig von Raumanzahl und Transportstrecke." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Wie kurzfristig ist ein Umzugstermin in Schierling verfügbar?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Da sich unsere Disposition im nahen Regensburg befindet, können wir oft auch sehr kurzfristige Umzüge oder Notfall-Termine in Schierling realisieren." } },
                { "@type": "Question", "name": content.faqs?.[2]?.q || "Kümmern Sie sich um Halteverbotszonen in Schierling?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[2]?.a || "Ja, absolut. Wenn Parkraum vor Ihrer Haustür knapp ist, organisieren wir die offizielle Beantragung sowie das Aufstellen der Halteverbotsschilder durch die lokalen Behörden." } },
                { "@type": "Question", "name": content.faqs?.[3]?.q || "Bieten Sie auch Küchenmontage in Schierling an?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[3]?.a || "Wir demontieren Ihre Küche fachmännisch und verpacken die Teile sicher. Den fachgerechten Wiederaufbau übernehmen wir im Rahmen der Machbarkeiten ebenfalls gerne." } },
    ...
    const localBusinessJsonLd = {
        ...
        "url": "https://www.floxant.de/" + pageLocale + "/umzug-schierling",
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Schierling", "item": "https://www.floxant.de/" + pageLocale + "/umzug-schierling" }
        ]
    };

    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Region Regensburg", href: "/" + pageLocale + "/umzug-regensburg" }, { label: "Umzug Schierling" }]} />
