...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return generatePageSEO({
        ...
        title: "Umzugsunternehmen Schwabach ✓ Festpreis ✓ Versichert | FLOXANT",
        description: "Ihr Umzugsunternehmen für Schwabach (Mittelfranken). Professioneller Umzug mit Festpreisgarantie, voller Versicherung und schneller Verfügbarkeit. Jetzt Angebot anfordern! Sofortpreis online berechnen oder bequem per WhatsApp / Telefon anfragen: +49 1577 1105087.",
    });
}
...
export default async function UmzugSchwabach({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
            { "@type": "Question", "name": "Was kostet ein Umzug in Schwabach?", "acceptedAnswer": { "@type": "Answer", "text": "Die Kosten hängen von Wohnungsgröße, Etage und Entfernung ab. Ein Studentenumzug startet bei günstigen Einstiegspreisen. Familien erhalten ein maßgeschneidertes Festpreisangebot nach kostenloser Besichtigung." } },
            { "@type": "Question", "name": "Wie schnell ist FLOXANT in Schwabach verfügbar?", "acceptedAnswer": { "@type": "Answer", "text": "Da unsere Teams täglich auf der Strecke A3 Regensburg–Nürnberg unterwegs sind, können wir oft schon innerhalb weniger Tage einen Termin in Schwabach realisieren." } },
            ...
            { "@type": "Question", "name": "Bieten Sie auch Entrümpelung in Schwabach an?", "acceptedAnswer": { "@type": "Answer", "text": "Ja. Wir kombinieren Umzug, Entrümpelung und Endreinigung zu einem Paket. Ideal bei Wohnungsauflösungen oder wenn die alte Wohnung besenrein übergeben werden muss." } },
        ],
    };
    ...
    const localBusinessJsonLd = {
        ...
        "url": "https://www.floxant.de/" + lang + "/umzug-schwabach",
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Schwabach", "item": "https://www.floxant.de/" + lang + "/umzug-schwabach" }
        ]
    };
    ...
    return (
        <main className="min-h-screen bg-background">
            ...
            <Breadcrumbs lang={lang} items={[{ label: "Umzug Bayern", href: "/" + lang + "/umzug-bayern" }, { label: "Umzug Schwabach" }]} />
                ...
                <div className="max-w-4xl mx-auto space-y-24">
                    ...
                    <div className="border-t border-border pt-16">
                        ...
                        <div className="flex flex-wrap justify-center gap-3">
                            ...
                            <Link href={"/" + lang + "/umzug-nuernberg"} className="px-5 py-3 rounded-2xl border border-border/50 bg-card text-sm font-semibold text-foreground hover:border-primary/50 transition-all shadow-sm">Umzug Nürnberg</Link>
                            <Link href={"/" + lang + "/umzug-herzogenaurach"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Herzogenaurach</Link>
                            ...
                            <Link href={"/" + lang + "/umzug-lauf-an-der-pegnitz"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Lauf an der Pegnitz</Link>
                            <Link href={"/" + lang + "/umzug-erding"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Erding</Link>
                            <Link href={"/" + lang + "/umzug-mering"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Mering</Link>
                            <Link href={"/" + lang + "/umzug-freystadt"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Freystadt</Link>
                        </div>
                    </div>
