...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return generatePageSEO({
        ...
        title: 'Umzugsunternehmen in Nürnberg | FLOXANT',
        description: 'Professionelle Umzugsunternehmen in Nürnberg in Bayern. Seriöse Abwicklung, Festpreisgarantie und versicherter Transport. Jetzt online berechnen!',
    });
}
...
export default async function UmzugNuernberg({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in Nürnberg?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Ein lokaler Umzug in Nürnberg kostet zwischen 400 und 2.000 Euro je nach Wohnungsgröße. FLOXANT bietet verbindliche Festpreise nach kostenloser Besichtigung." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Macht FLOXANT auch Umzüge in der Altstadt von Nürnberg?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Ja, wir sind auf schwierige Logistik wie enge Gassen oder viele Treppen spezialisiert und kümmern uns um alle Halteverbotszonen." } },
                { "@type": "Question", "name": content.faqs?.[2]?.q || "Bieten Sie Fernumzüge ab Nürnberg an?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[2]?.a || "Ja. Wir organisieren Fernumzüge von Nürnberg nach ganz Deutschland." } }
            ],
    };
    ...
    const localBusinessJsonLd = {
        ...
        "description": "Professionelles Umzugsunternehmen. Moving company and clearance services in Nürnberg. Local moving, long distance, and professional packing.",
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Nürnberg", "item": "https://www.floxant.de/" + pageLocale + "/umzug-nuernberg" }
        ]
    };

    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Bayern", href: "/" + pageLocale + "/umzug-bayern" }, { label: "Umzug Nürnberg" }]} />
            ...
            <section className="py-20 px-6">
                <div className="max-w-4xl mx-auto space-y-24">
                    ...
                    <div className="prose prose-lg max-w-none text-muted-foreground">
                        ...
                        <p>Wer nach Nürnberg zieht, oder die Stadt verlässt, muss oft auch alte Möbel entsorgen. FLOXANT bietet exzellente <Link href={"/" + pageLocale + "/entruempelung"}>{dict.common.entruempelung_services}</Link>{dict.common.waste_disposal_desc}</p>
                    ...
                    <div className="bg-muted/20 p-8 rounded-3xl border border-border/50 text-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left max-w-3xl mx-auto">
                            <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                                ...
                                <p className="font-semibold">– Premium Dienstleister, Nürnberg</p>
                            </div>
                            <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
