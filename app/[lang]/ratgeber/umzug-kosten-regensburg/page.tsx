...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return generatePageSEO({
        ...
        title: 'Was kostet ein Umzug in Regensburg? | FLOXANT',
        description: 'Professionelle Was kostet ein Umzug in Regensburg? in Bayern. Seriöse Abwicklung, Festpreisgarantie und versicherter Transport. Jetzt online berechnen!',
    });
}
...
export default async function Article({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in Regensburg?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Zwischen 400 und 2.500 Euro je nach Wohnungsgröße und Service." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Gibt es Festpreise?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Ja. FLOXANT bietet nach kostenloser Besichtigung ein verbindliches Festpreisangebot." } }
            ],
    };
    ...
    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Ratgeber", href: `/${pageLocale}/ratgeber` }, { label: "Umzug Kosten Regensburg 2026" }]} />
