...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const { seoContent, seoFallback, city } = await getSpecialtyPageData({
        ...
        seoKey: "reinigung_erlangen",
        city: "Erlangen",
    });

    return generatePageSEO({
        pageLocale: lang,
        path: `reinigung-erlangen`,
...
export default async function ReinigungCityPage({ params }: PageProps) {
    ...
    const jsonLd = {
        ...
        "@graph": [
            {
                "@type": "Service",
                "name": `Endreinigung ${city} | FLOXANT`,
                "description": resolveField(seoContent.meta_desc, seoFallback.meta_desc, city),
                "provider": {
                    "@type": "MovingCompany",
                    "name": "FLOXANT",
            ...
            },
            {
                ...
                "itemListElement": [
                    ...
                    { "@type": "ListItem", "position": 3, "name": city, "item": `https://www.floxant.de/${lang}/reinigung-erlangen` }
                ]
            },
            ...(faqItems.length > 0 ? [{
                "@type": "FAQPage",
                    ...
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {[
                                ...
                                { name: "Weiden", href: "/de/reinigung-weiden" }
                            ].map((loc) => (
                                <Link 
                                    key={loc.name} 
                                    href={loc.href}
