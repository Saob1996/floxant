...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return generatePageSEO({
        lang,
        path: "umzug-neufahrn-in-niederbayern",
        title: "Umzug in Neufahrn in Niederbayern ab 79€ – Sofort verfügbar | FLOXANT",
        description: "Professionelles Umzugsunternehmen für Neufahrn in Niederbayern. ✓ Festpreisgarantie ✓ 100% Versichert ✓ Möbelmontage. Mehr als 100+ erfolgreiche Umzüge. Jetzt Angebot sichern! Sofortpreis online berechnen oder bequem per WhatsApp / Telefon anfragen: +49 1577 1105087.",
    });
}

export default async function UmzugNeufahrninniederbayern({ params }: { params: Promise<{ lang: string }> }) {
    const { lang } = await params;
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
            { "@type": "Question", "name": "Was kostet ein Umzug in Neufahrn in Niederbayern?", "acceptedAnswer": { "@type": "Answer", "text": "Ein Transporter mit Helfern ist oft schon ab günstigen Einstiegspreisen verfügbar. Für den durchschnittlichen Familienhaushalt kalkulieren wir maßgeschneiderte Festpreise, in der Regel zwischen 400€ und 1.500€, abhängig von Raumanzahl und Transportstrecke." } },
            { "@type": "Question", "name": "Wie kurzfristig ist ein Umzugstermin in Neufahrn in Niederbayern verfügbar?", "acceptedAnswer": { "@type": "Answer", "text": "Da sich unsere Disposition im nahen Regensburg befindet, können wir oft auch sehr kurzfristige Umzüge oder Notfall-Termine in Neufahrn in Niederbayern realisieren." } },
            { "@type": "Question", "name": "Kümmern Sie sich um Halteverbotszonen in Neufahrn in Niederbayern?", "acceptedAnswer": { "@type": "Answer", "text": "Ja, absolut. Wenn Parkraum vor Ihrer Haustür knapp ist, organisieren wir die offizielle Beantragung sowie das Aufstellen der Halteverbotsschilder durch die lokalen Behörden." } },
            { "@type": "Question", "name": "Bieten Sie auch Küchenmontage in Neufahrn in Niederbayern an?", "acceptedAnswer": { "@type": "Answer", "text": "Wir demontieren Ihre Küche fachmännisch und verpacken die Teile sicher. Den fachgerechten Wiederaufbau übernehmen wir im Rahmen der Machbarkeiten ebenfalls gerne." } },
    ...
    const localBusinessJsonLd = {
        ...
        "name": "FLOXANT Umzug Neufahrn in Niederbayern",
        "description": "Professioneller Umzugsservice, Möbeltransport und Entrümpelung in Neufahrn in Niederbayern. Regional, versichert und zum garantierten Festpreis.",
        "url": "https://www.floxant.de/" + lang + "/umzug-neufahrn-in-niederbayern",
        "telephone": "+4915771105087",
        "address": { "@type": "PostalAddress", "addressLocality": "Neufahrn in Niederbayern", "addressRegion": "Bayern", "addressCountry": "DE" },
        "areaServed": [{ "@type": "City", "name": "Neufahrn in Niederbayern" }, { "@type": "City", "name": "Regensburg" }],
        "priceRange": "$$",
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Neufahrn in Niederbayern", "item": "https://www.floxant.de/" + lang + "/umzug-neufahrn-in-niederbayern" }
        ]
    };
    ...
    return (
        <main className="min-h-screen bg-background">
            ...
            <Breadcrumbs lang={lang} items={[{ label: "Umzug Region Regensburg", href: "/" + lang + "/umzug-regensburg" }, { label: "Umzug Neufahrn in Niederbayern" }]} />
            ...
            <section className="pt-12 pb-24 px-6 bg-gradient-to-b from-primary/5 via-muted/30 to-background overflow-hidden relative">
                ...
                <div className="max-w-7xl mx-auto text-center space-y-8 relative z-10">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-sm font-bold shadow-sm ring-1 ring-primary/20 justify-center">
                        <MapPin className="w-4 h-4" /><span>Ihr Umzugspartner für Neufahrn in Niederbayern & Umgebung</span>
                    </div>
                    <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-foreground leading-tight">
                        Stressfreier Umzug in <br className="hidden md:block"/>
                        <span className="text-primary bg-clip-text text-transparent bg-gradient-to-r from-primary to-blue-600">Neufahrn in Niederbayern</span>
                    </h1>
                ...
                <div className="max-w-4xl mx-auto space-y-24">
                    ...
                    <div>
                        <div className="text-center mb-16">
                            <h2 className="text-4xl font-extrabold text-foreground mb-4">Warum FLOXANT in Neufahrn in Niederbayern?</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
                            ...
                            <div className="p-8 rounded-3xl bg-card border border-border shadow-md hover:shadow-lg transition-all group">
                                ...
                                <p className="text-muted-foreground leading-relaxed">Vertrauen ist gut, Versicherungen sind besser. Vom Ladevorgang in Neufahrn in Niederbayern bis zum Entladen in der Zieladresse haftet unsere Betriebshaftpflicht für eventuelle Schäden – so können Sie nachts ruhig schlafen.</p>
                            </div>
                    ...
                    <div className="bg-card p-8 md:p-12 rounded-[2.5rem] border border-border shadow-sm">
                        <div className="text-center mb-10">
                            <h2 className="text-3xl font-extrabold text-foreground mb-4">Häufige Fragen zum Standort Neufahrn in Niederbayern</h2>
                        ...
                        <div className="space-y-4 max-w-3xl mx-auto">
                            <details className="group border border-border/50 rounded-2xl p-6 bg-muted/20 open:bg-card open:ring-2 open:ring-primary/20 transition-all cursor-pointer">
                                <summary className="text-xl font-bold list-none flex justify-between items-center outline-none">
                                    <span>Was kostet ein durchschnittlicher Umzug in Neufahrn in Niederbayern?</span>
                            ...
                            <details className="group border border-border/50 rounded-2xl p-6 bg-muted/20 open:bg-card open:ring-2 open:ring-primary/20 transition-all cursor-pointer">
                                <summary className="text-xl font-bold list-none flex justify-between items-center outline-none">
                                    <span>Können Umzugskartons in Neufahrn in Niederbayern geliefert werden?</span>
                                    <span className="text-primary transition-transform group-open:rotate-180">▼</span>
                                </summary>
                                <div className="pt-5 text-muted-foreground leading-relaxed">Wenn Sie unseren Einpack-Service nicht buchen, können wir Ihnen Vorab hochwertiges Verpackungsmaterial (stabile Kartons, Kleiderboxen) bequem direkt an Ihre Haustür in Neufahrn in Niederbayern liefern.</div>
                            </details>
                            <details className="group border border-border/50 rounded-2xl p-6 bg-muted/20 open:bg-card open:ring-2 open:ring-primary/20 transition-all cursor-pointer">
                                <summary className="text-xl font-bold list-none flex justify-between items-center outline-none">
                    ...
                    <div className="border-t border-border pt-16">
                        ...
                        <div className="flex flex-wrap justify-center gap-3">
                            ...
                            <Link href={"/" + lang + "/umzug-abensberg"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Abensberg</Link>
                            <Link href={"/" + lang + "/umzug-koefering"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Köfering</Link>
                            <Link href={"/" + lang + "/umzug-sinzing"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Sinzing</Link>
                            <Link href={"/" + lang + "/umzug-geiselhoering"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Geiselhöring</Link>
                            <Link href={"/" + lang + "/umzug-pfeffenhausen"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Pfeffenhausen</Link>
                            <Link href={"/" + lang + "/umzug-kelheim"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Kelheim</Link>
                            <Link href={"/" + lang + "/umzug-langquaid"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Langquaid</Link>
                            <Link href={"/" + lang + "/umzug-neustadt-an-der-donau"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Neustadt an der Donau</Link>
                            <Link href={"/" + lang + "/umzug-aufhausen"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Aufhausen</Link>
                            <Link href={"/" + lang + "/umzug-maxhuette-haidhof"} className="px-4 py-2 rounded-full border border-border/50 text-sm font-medium text-muted-foreground hover:text-primary hover:border-primary/50 transition-all bg-card/50">Umzug Maxhütte-Haidhof</Link>
                        </div>
                    </div>
                    ...
                    <div id="wizard" className="text-center py-16 bg-card rounded-[3rem] border border-border shadow-2xl relative mt-16 scroll-mt-24">
                        ...
                        <p className="text-lg text-muted-foreground mb-12 max-w-2xl mx-auto">Sichern Sie sich jetzt Ihren Umzugstermin für Neufahrn in Niederbayern. Wir berechnen das Volumen und das beste Angebot für Sie in Kürzester Zeit.</p>
                        <div className="px-6">
                            <DualCalculator />
                        </div>
                    </div>
