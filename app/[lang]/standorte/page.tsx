...
import { MapPin, ArrowRight, Search, Globe, ChevronRight } from "lucide-react";
import { getDictionary } from "../../../get-dictionary";
import { isValidLocale, type Locale } from "@/i18n-config";
import { notFound } from "next/navigation";
import { BAVARIAN_CITIES_GEO, type CityGeoData } from "@/lib/geo-data";
import { SERVICE_CITY_PAGES } from "@/lib/sitemap-config";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return generatePageSEO({
        lang,
        path: "/standorte",
        title: "Standorte & Servicegebiete | FLOXANT National",
        description: "Alle FLOXANT Standorte in Bayern und ganz Deutschland. Finden Sie Ihren professionellen Partner für Umzug, Reinigung und Logistik direkt in Ihrer Nähe.",
    });
}

export default async function StandortePage({ params }: PageProps) {
    const { lang } = await params;
        ...
        <main className="min-h-screen bg-background">
            ...
            <section className="relative overflow-hidden bg-slate-950 py-24 px-6">
                ...
                <div className="relative z-10 max-w-7xl mx-auto text-center md:text-start">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-white/80 text-xs font-bold uppercase tracking-widest mb-8">
                        ...
                        Nationales Logistiknetzwerk
                    </div>
                    <h1 className="text-5xl md:text-7xl font-extrabold text-white tracking-tight mb-6">
                        Unsere <span className="text-primary">Standorte</span>
                    </h1>
                    <p className="text-xl text-white/60 max-w-3xl leading-relaxed">
                        Von unserem Kernland Bayern bis hin zu den großen Metropolen Deutschlands – FLOXANT ist dort, wo Sie uns brauchen. Entdecken Sie unser flächendeckendes Netzwerk für Umzüge und Logistik.
                    </p>
                </div>
            </section>
            ...
            <section className="relative -mt-12 px-6">
                <div className="max-w-4xl mx-auto bg-card border border-border rounded-3xl shadow-2xl p-4 flex items-center gap-4">
                    ...
                    />
                    <button className="hidden md:block px-8 py-3 bg-primary text-white font-bold rounded-2xl shadow-lg shadow-primary/20 hover:scale-105 transition-all">
                        Suchen
                    </button>
                </div>
            </section>
            ...
            <section className="py-24 px-6 bg-secondary/30">
                <div className="max-w-4xl mx-auto text-center space-y-8">
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        <Link href="/de/angebote" className="px-10 py-5 bg-primary text-white font-black rounded-full shadow-2xl shadow-primary/30 hover:scale-105 transition-all flex items-center gap-3">
