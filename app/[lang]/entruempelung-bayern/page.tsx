...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const safeTitle = (content.meta_title || "").replace('{city}', 'Bayern');
    const safeDesc = (content.meta_desc || "").replace('{city}', 'Bayern');
...
export default async function GenericServicePage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const serviceName = (content.hero_h1 || "Service") + " Bayern";
    const serviceDesc = (content.meta_desc || "").replace('{city}', 'Bayern');
    ...
    const serviceJsonLd = {
        ...
        "areaServed": { "@type": "City", "name": "Bayern" },
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": serviceName, "item": "https://www.floxant.de/" + pageLocale + "/entruempelung-bayern" }
        ]
    };
    ...
    return (
        <main className="min-h-screen bg-background">
            ...
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: content.link_umzug?.replace('{city}', 'Bayern') || "Umzug Bayern", href: "/" + pageLocale + "/umzug-bayern" }, { label: content.hero_h1 || "Service" }]} />
            ...
            <section className="pt-12 pb-24 px-6 bg-gradient-to-b from-slate-100 dark:from-slate-900/40 via-muted/30 to-background overflow-hidden relative text-start">
                <div className="max-w-7xl mx-auto text-center space-y-8 relative z-10">
                    ...
                    <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-foreground leading-tight">
                        ...
                        <span className="text-primary"> Bayern</span>
                    </h1>
            ...
            <section className="py-24 px-6 text-start">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="prose prose-xl max-w-none text-muted-foreground leading-loose">
                        <h2 className="text-3xl font-extrabold text-foreground">{(content.section2_h2 || "").replace('{city}', 'Bayern')}</h2>
                        <p>{(content.section2_p1 || "").replace('{city}', 'Bayern')}</p>
                        <p>{(content.section2_p2 || "").replace('{city}', 'Bayern')}</p>
                    </div>
                    ...
                    <div id="wizard" className="text-center py-16 bg-card rounded-[3rem] border border-border shadow-2xl relative mt-16 scroll-mt-24">
                        ...
                        <h2 className="text-4xl font-extrabold mb-6 mt-6">{(content.wizard_h2 || "").replace('{city}', 'Bayern')}</h2>
