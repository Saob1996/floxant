...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    const content = dict?.pages?.umzug_straubing || {};
    return generatePageSEO({
        pageLocale,
        path: "umzug-straubing",
        title: content.meta_title || "Umzugsunternehmen Straubing | FLOXANT",
        description: "Professionelles Umzugsunternehmen in Straubing. Umzug, Entrümpelung und Reinigung mit Festpreis und Versicherung. Jetzt Angebot bei FLOXANT anfragen!",
    });
}
...
export default async function UmzugStraubing({ params }: { params: Promise<{ lang: string }> }) {
    ...
    const faqJsonLd = {
        ...
        "mainEntity": [
                { "@type": "Question", "name": content.faqs?.[0]?.q || "Was kostet ein Umzug in Straubing?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[0]?.a || "Ein lokaler Umzug in Straubing kostet zwischen 400 und 2.000 Euro je nach Wohnungsgröße. FLOXANT bietet verbindliche Festpreise nach kostenloser Besichtigung." } },
                { "@type": "Question", "name": content.faqs?.[1]?.q || "Macht FLOXANT auch Umzüge in der Altstadt von Straubing?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[1]?.a || "Ja, wir sind auf schwierige Logistik wie enge Gassen oder viele Treppen spezialisiert und kümmern uns um alle Halteverbotszonen." } },
                { "@type": "Question", "name": content.faqs?.[2]?.q || "Bieten Sie Fernumzüge ab Straubing an?", "acceptedAnswer": { "@type": "Answer", "text": content.faqs?.[2]?.a || "Ja. Wir organisieren Fernumzüge von Straubing nach ganz Deutschland." } }
            ],
    };

    const localBusinessJsonLd = {
        "@context": "https://schema.org", "@type": "MovingCompany",
        "name": "FLOXANT Umzug Straubing",
        "description": "Professionelle Umzugsfirma in Straubing – Privatumzüge, Firmenumzüge, Entrümpelungen.",
        "url": "https://www.floxant.de/" + pageLocale + "/umzug-straubing",
        "telephone": "+4915771105087",
        "address": { "@type": "PostalAddress", "addressLocality": "Straubing", "addressRegion": "Bayern", "addressCountry": "DE" },
        "geo": { "@type": "GeoCoordinates", "latitude": 48.881, "longitude": 12.5694 },
        "areaServed": [{ "@type": "City", "name": "Straubing" }, { "@type": "AdministrativeArea", "name": "Niederbayern" }],
        "priceRange": "$$",
    ...
    const serviceJsonLd = {
        ...
        "provider": { "@type": "LocalBusiness", "name": "FLOXANT Umzug Straubing", "telephone": "+4915771105087" },
        "areaServed": { "@type": "City", "name": "Straubing" }
    };
    ...
    const breadcrumbsJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "Umzug Straubing", "item": "https://www.floxant.de/" + pageLocale + "/umzug-straubing" }
        ]
    };

    return (
        <main className="min-h-screen bg-background">
            <Breadcrumbs pageLocale={pageLocale} items={[{ label: "Umzug Bayern", href: "/" + pageLocale + "/umzug-bayern" }, { label: "Umzug Straubing" }]} />
      ...
      <section className="py-20 px-6">
                <div className="max-w-4xl mx-auto space-y-24">
                    <div className="prose prose-lg max-w-none text-muted-foreground">
                        <h2 className="text-3xl font-bold text-foreground mb-6">Ihr verlässlicher Umzug in Straubing</h2>
                        <p>In der Gäubodenstadt Straubing bieten wir Full-Service-Umzüge an. Sparen Sie sich das Kistenschleppen in Straubing-Süd oder Kagers. Von der Planung bis zur finalen Möbelmontage – alles aus einer Hand zum transparenten Festpreis.</p>
                        <p>Ein Umzug ist mehr als nur der Transport von Kartons. Es ist ein Neustart. Wir von FLOXANT verstehen das und setzen alles daran, Ihren Wohnungswechsel so stressfrei wie möglich zu gestalten. Neben dem reinen Möbeltransport bieten wir Ihnen in Straubing auch professionelle Demontage- und Montagearbeiten, Einpackservice sowie die Bereitstellung von hochwertigem Verpackungsmaterial an.</p>
                        <div className="bg-muted/30 p-6 rounded-xl border border-border/50 not-italic mt-6">
                            ...
                            <p className="m-0 text-sm">Nach unser kostenlosen, oft virtuellen Besichtigung erhalten Sie ein verbindliches Festpreisangebot. Keine versteckten Gebühren, keine Stundensätze, die aus dem Ruder laufen. Sie wissen genau, was Ihr Umzug in Straubing kosten wird.</p>
                        </div>
                    </div>

                    <div>
                        <h2 className="text-3xl font-bold text-foreground mb-12">Unsere Kompetenzen für Straubing</h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="p-6 rounded-2xl bg-card border border-border shadow-sm">
                                ...
                                <p className="text-sm text-muted-foreground">Kenntnis der örtlichen Gegebenheiten, Treppenhäuser und Halteverbots-Beantragung direkt in Straubing.</p>
                            </div>
                            <div className="p-6 rounded-2xl bg-card border border-border shadow-sm">
                    ...
                    <div className="prose prose-lg max-w-none text-muted-foreground">
                        ...
                        <p>Wer nach Straubing zieht, oder die Stadt verlässt, muss oft auch alte Möbel entsorgen. FLOXANT bietet exzellente <Link href={"/" + pageLocale + "/entruempelung"}>{dict.common.entruempelung_services}</Link>{dict.common.waste_disposal_desc}</p>
                    ...
                    {/* FAQ Section */}
                    <div>
                        <h2 className="text-3xl font-bold text-foreground mb-8">Häufige Fragen zum Umzug in Straubing</h2>
                        <div className="space-y-4">
                            <details className="group border border-border/50 rounded-lg p-4 bg-muted/10 open:ring-2 open:ring-primary/20 transition-all">
                                <summary className="text-lg font-medium cursor-pointer list-none flex justify-between items-center outline-none"><span>Was kostet ein Umzug in Straubing?</span><span className="transition group-open:rotate-180">↓</span></summary>
                            ...
                            <details className="group border border-border/50 rounded-lg p-4 bg-muted/10 open:ring-2 open:ring-primary/20 transition-all">
                                ...
                                <div className="pt-4 text-muted-foreground">Falls am Be- oder Entladeort in Straubing keine Parkplätze verfügbar sind, übernehmen wir die behördliche Anmeldung und Beschilderung der offiziellen Halteverbotszone für Sie komplett.</div>
                            </details>
                        </div>
                    </div>
                    ...
                    <div className="bg-muted/20 p-8 rounded-3xl border border-border/50 text-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left max-w-3xl mx-auto">
                            <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                                <p className="italic text-muted-foreground mb-4">"Wir sind als Familie nach Straubing gezogen. Der Einpackservice war Gold wert, wir konnten entspannt zusehen."</p>
                                <p className="font-semibold">– Familie S.</p>
                            </div>
                            <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                    ...
                    <div className="text-center py-10 bg-gradient-to-br from-primary/5 via-transparent to-primary/5 rounded-3xl border border-primary/10 shadow-lg mt-8">
                        ...
                        <p className="text-muted-foreground mb-8 max-w-xl mx-auto">Kontaktieren Sie uns noch heute. Wir erstellen Ihr individuelles Festpreisangebot für Straubing.</p>
