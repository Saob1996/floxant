...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return generatePageSEO({
        lang: "de",
        path: "wohnungsaufloesung-muenchen",
...
export default async function WohnungsaufloesungMuenchenPage({ params }: PageProps) {
    const locale = "de";
    const { 
        localeDict, 
        content, 
    ...
    return (
        <SpecialtyPageLayout
            pagePath="/wohnungsaufloesung-muenchen"
                lang="de"
                dict={localeDict}
                city={city}
                heroBadge={resolveField(content.hero_badge, fallback.hero_badge, city, "de")}
        heroTitle="Wohnungsauflösung in München anfragen"
                heroText={resolveField(content.hero_p, fallback.hero_p, city, "de")}
                ctaText={resolveField(content.cta, fallback.cta, city, "de")}
                breadcrumbs={[{"label":"Home","href":"/"},{"label":"Entrümpelung","href":"/entruempelung"},{"label":"Wohnungsauflösung","href":"/wohnungsaufloesung-bayern"},{"label":"München"}]}
                chips={[
