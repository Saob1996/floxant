...
import { Database, ShieldCheck, Clock, Archive, Scan } from "lucide-react";
interface PageProps {
  params: Promise<{}>;
}
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { seoContent, seoFallback, city } = await getSpecialtyPageData({
    locale: "de",
    baseKey: "akteneinlagerung_regensburg_spec",
    city: "Regensburg",
  });
  return generatePageSEO({
    lang: "de",
    path: "akteneinlagerung-regensburg",
    title: "Akteneinlagerung Regensburg | Abholung & Lagerung | FLOXANT",
...
export default async function AkteneinlagerungRegensburgPage({ params }: PageProps) {
  ...
  return (
    <SpecialtyPageLayout
        ...
        chips={[
          { icon: Database, text: resolveNestedField(content.badges, fallback.badges, "permit", city) },
          { icon: ShieldCheck, text: resolveNestedField(content.badges, fallback.badges, "signs", city) },
